...
extern fn Bun__generateHeapProfile(vm: *support.VM) support.BunString;
extern fn Bun__generateHeapSnapshotV8(vm: *support.VM) support.BunString;

fn implementation(string: support.BunString) !*support.WTFStringImpl {
    if (string.tag != .wtf_string_impl) return error.ExpectedOwnedString;
    return string.value.wtf_string_impl orelse error.ExpectedOwnedString;
}
...
pub fn main() !void {
    ...
    const first_string = Bun__generateHeapSnapshotV8(fixture.vm);
    const first_impl = try implementation(first_string);
    ...
    const second_string = Bun__generateHeapSnapshotV8(fixture.vm);
    const second_impl = try implementation(second_string);
    ...
    const profile_string = Bun__generateHeapProfile(fixture.vm);
    const profile_impl = try implementation(profile_string);
    ...
    const before_string = Bun__generateHeapSnapshotV8(gc_fixture.vm);
    const before_impl = try implementation(before_string);
    ...
    const after_string = Bun__generateHeapSnapshotV8(gc_fixture.vm);
    const after_impl = try implementation(after_string);
