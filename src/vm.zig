...
pub const Frame = struct {
    ...
    /// Lazily published variable Environment for this activation's direct eval.
...
fn sameDirectEvalBindings(left: []const bc.DirectEvalBinding, right: []const bc.DirectEvalBinding) bool {
    ...
    for (left, right) |a, b| {
        if (!std.mem.eql(u8, a.name, b.name) or
            a.slot != b.slot or
            a.lexical != b.lexical or
            a.immutable != b.immutable or
            a.tdz_checked != b.tdz_checked or
            a.mapped_parameter != b.mapped_parameter)
            return false;
    }
    return true;
}

fn directEvalFunctionEnvironmentMatches(
...
fn reparentUnpublishedDirectEvalEnvironment(environment: *Environment, parent: *Environment) void {
    ...
    if (environment.gc_managed and parent.gc_managed)
        if (!gc_mod.barrierExactManagedCellFrom(environment, parent))
            gc_mod.barrierCellFrom(environment, parent);
}

fn publishDirectEvalFunctionEnvironment(
    vm: *Interpreter,
    frame: *Frame,
    scope: bc.DirectEvalScope,
    candidate: *Environment,
) EvalError!*Environment {
    // Frame is an external precise root rather than a managed owner. Shade the
...
/// Lazily install declarative Environment Records whose bindings are live views
/// over an ordinary VM activation. The compiler orders scopes outer-to-inner;
/// validation walks it inner-to-outer once, then allocation links the verified
/// scopes directly from their defining closure. Each defining frame's variable
/// Environment is retained for later sloppy eval declarations, while call-site
/// lexical wrappers remain transient. The complete plan and allocation set are
/// prepared before publication, so recoverable failure leaves every frame cache
/// and `vm.env` unchanged.
fn materializeDirectEvalEnvironment(
    vm: *Interpreter,
    current_frame: *Frame,
    plan: *const bc.DirectEvalPlan,
) EvalError!MaterializedDirectEvalEnvironment {
    if (plan.scopes.len == 0)
    ...
    while (scope_index > 0) {
        scope_index -= 1;
        const scope = plan.scopes[scope_index];
        ...
        for (scope.bindings) |binding| {
            if (binding.slot >= selected_frame.slots.len or binding.name.len == 0 or binding.name[0] == 0)
                return vm.throwError("InternalError", "invalid direct-eval activation binding");
            if (previous_name) |previous|
                if (std.mem.order(u8, previous, binding.name) != .lt)
                    return vm.throwError("InternalError", "unsorted direct-eval activation bindings");
    ...
    if (plan.current_environment_depth == 0) if (target_cache.load(.acquire)) |cached| {
        ...
        const function_root = try vm.pushTempEnvRoot(cached);
    ...
    const frames = try vm.arena.alloc(*Frame, frame_count);
    ...
    const base_root = try vm.pushTempEnvRoot(base_environment);
    defer vm.restoreTempEnvRoots(base_root);
    const chain_root = try vm.pushTempEnvRoot(vm.tempEnvRoot(base_root, base_environment));
    const function_root = try vm.pushTempEnvRoot(vm.tempEnvRoot(base_root, base_environment));
    const scope_roots_mark = try vm.pushTempEnvRoot(vm.tempEnvRoot(base_root, base_environment));
    ...
    // a later outer eval reuses it instead of leaving the inner cache attached
    // to a stale sibling. Concurrent contenders converge through CAS. When one
...
test "vm materializes direct eval environments over exact defining frames failure atomically" {
    ...
    const PublishBarrier = struct {
        ...
        child: ?*anyopaque = undefined,

        fn barrier(raw: *anyopaque, owner: ?*anyopaque, child: ?*anyopaque) void {
    ...
    var publish_barrier = PublishBarrier{};
    const materialized = materialized: {
        const previous_barrier = gc_runtime.setBarrier(&publish_barrier, PublishBarrier.barrier, null, null);
        defer _ = gc_runtime.setBarrier(previous_barrier[0], previous_barrier[1], previous_barrier[2], previous_barrier[3]);
        break :materialized try materializeDirectEvalEnvironment(&machine, &inner, &plan);
    };
    const installed = materialized.environment;
    try std.testing.expectEqual(@as(usize, 2), publish_barrier.calls);
    try std.testing.expectEqual(@as(?*anyopaque, null), publish_barrier.owner);
    try std.testing.expectEqual(@intFromPtr(materialized.function_environment), @intFromPtr(publish_barrier.child.?));
    try std.testing.expectEqual(installed, machine.env);
    try std.testing.expectEqual(inner.direct_eval_environment.load(.acquire).?, materialized.function_environment);
    try std.testing.expect(!installed.fn_scope);
    try std.testing.expectEqual(@as(f64, 30), installed.get("lexical").?.asNum());
    try std.testing.expectEqual(@as(?bool, true), installed.isConst("lexical"));
    const inner_environment = installed.parent.?;
    try std.testing.expect(inner_environment.fn_scope);
    try std.testing.expectEqual(@as(f64, 20), inner_environment.getLocal("inner").?.asNum());
    try std.testing.expectEqual(inner_environment, installed.varScope());
    const outer_environment = inner_environment.parent.?;
    try std.testing.expect(outer_environment.fn_scope);
    try std.testing.expectEqual(outer_environment, outer.direct_eval_environment.load(.acquire).?);
    try std.testing.expectEqual(@as(f64, 10), outer_environment.getLocal("outer").?.asNum());
    try std.testing.expectEqual(&root, outer_environment.parent.?);

    try installed.assign("outer", Value.num(41));
    try std.testing.expectEqual(@as(f64, 41), outer_slots[0].asNum());
    installed.markCaptured();
    try std.testing.expect(inner.escaped.load(.acquire));
    try std.testing.expect(outer.escaped.load(.acquire));
    ...
    try std.testing.expectEqual(@as(?*Environment, null), cold_inner.direct_eval_environment.load(.acquire));
    try std.testing.expectEqual(&root, machine.env);
    ...
    for (0..16) |fail_index| {
        ...
        machine.env = &root;
        const attempt = materializeDirectEvalEnvironment(&machine, &transactional_inner, &plan) catch |err| {
            try std.testing.expectEqual(error.OutOfMemory, err);
            try std.testing.expectEqual(@as(?*Environment, null), transactional_outer.direct_eval_environment.load(.acquire));
            try std.testing.expectEqual(@as(?*Environment, null), transactional_inner.direct_eval_environment.load(.acquire));
            try std.testing.expectEqual(&root, machine.env);
            continue;
        };
...
test "vm activation eval opcode reads and mutates the exact live slot" {
    ...
    const first_source_index = try chunk.addConst(Value.str("var dynamic = 1; local += dynamic; local"));
    const second_source_index = try chunk.addConst(Value.str("dynamic += 1; local += dynamic; local"));
    const plan_index = try chunk.addDirectEvalPlan(.{ .scopes = &scopes });
    _ = try chunk.emit(.load_const, eval_index);
    _ = try chunk.emit(.load_const, first_source_index);
    _ = try chunk.emitAB(.call_eval_activation, 1, plan_index);
    _ = try chunk.emit(.pop, 0);
    _ = try chunk.emit(.load_const, eval_index);
    _ = try chunk.emit(.load_const, second_source_index);
    _ = try chunk.emitAB(.call_eval_activation, 1, plan_index);
    _ = try chunk.emit(.ret, 0);
    try chunk.finalize();

    const result = try run(&machine, &chunk, &frame);
    try std.testing.expectEqual(@as(f64, 44), result.asNum());
    try std.testing.expectEqual(@as(f64, 44), slots[0].asNum());
    try std.testing.expectEqual(frame.direct_eval_environment.load(.acquire).?, machine.env);
    try std.testing.expectEqual(@as(f64, 2), machine.env.get("dynamic").?.asNum());
    try std.testing.expectEqual(&root, machine.env.parent.?);
    try std.testing.expect(!machine.direct_eval_call);

    var syntax_slots = [_]Value{Value.num(0)};
    var syntax_frame = Frame{ .slots = &syntax_slots, .parent = null };
    machine.env = &root;
    try std.testing.expectError(error.Throw, callActivationEvalValue(
        &machine,
        &chunk,
        &syntax_frame,
        null,
    ...
    var identity_slots = [_]Value{Value.num(0)};
    var identity_frame = Frame{ .slots = &identity_slots, .parent = null };
    machine.env = &root;
    var unavailable = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    machine.arena = unavailable.allocator();
    const identity = try callActivationEvalValue(
        &machine,
        &chunk,
        &identity_frame,
        null,
...
test "vm: own arguments default allocation failure restores the caller activation" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    var parser = try Parser.init(
        allocator,
        "function own(value = arguments) { return value.length; } own",
    );
    const program = try parser.parseProgram();
    const root = try Compiler.compileProgram(allocator, program);
    var env = Environment{ .arena = allocator, .fn_scope = true };
    ...
    const function_chunk = function.chunk orelse return error.TestUnexpectedResult;
    try std.testing.expectEqualSlices(u32, &.{0}, function_chunk.default_parameter_indices);
    try std.testing.expect(function_chunk.arguments_slot != null);

    try std.testing.expectEqual(
        @as(f64, 0),
        (try runFunction(&machine, function, function_chunk, &.{}, Value.undef(), Value.undef())).asNum(),
    );
    try std.testing.expect(machine.vm_activation_free != null);

    const saved_this = Value.num(101);
    const saved_new_target = Value.num(102);
    ...
    for (0..32) |fail_index| {
        ...
        machine.arena = allocator;
        if (result) |value_out| {
            try std.testing.expectEqual(@as(f64, 0), value_out.asNum());
            recovered = true;
        } else |err| {
            try std.testing.expectEqual(error.OutOfMemory, err);
            failures += 1;
...
test "vm: parameter-safe string default allocation failure restores the caller activation" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    var parser = try Parser.init(
        allocator,
        "function compose(first, value = first + first) { return value; } compose",
    );
    const program = try parser.parseProgram();
    const root = try Compiler.compileProgram(allocator, program);
    var env = Environment{ .arena = allocator, .fn_scope = true };
    ...
    const function = Interpreter.funcOf(function_value) orelse return error.TestUnexpectedResult;
    const function_chunk = function.chunk orelse return error.TestUnexpectedResult;
    try std.testing.expectEqualSlices(u32, &.{1}, function_chunk.default_parameter_indices);
    const input = try Value.strAlloc(allocator, "parameter-safe");
    const arguments = [_]Value{input};

    try std.testing.expectEqualStrings(
        "parameter-safeparameter-safe",
        (try runFunction(&machine, function, function_chunk, &arguments, Value.undef(), Value.undef())).asStr(),
    );
    try std.testing.expect(machine.vm_activation_free != null);

    const saved_this = Value.num(111);
    const saved_new_target = Value.num(112);
    var failures: usize = 0;
    var recovered = false;
    defer machine.arena = allocator;
    for (0..16) |fail_index| {
        ...
        machine.arena = allocator;
        if (result) |value_out| {
            try std.testing.expectEqualStrings("parameter-safeparameter-safe", value_out.asStr());
            recovered = true;
        } else |err| {
            try std.testing.expectEqual(error.OutOfMemory, err);
            failures += 1;
...
test "vm: parameter property getter allocation failure restores the caller activation" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    var parser = try Parser.init(
        allocator,
        "var propertyInput = { get value() { return 'parameter' + '-property'; } }; function read(first, value = first.value) { return value; } read",
    );
    const program = try parser.parseProgram();
    const root = try Compiler.compileProgram(allocator, program);
    var env = Environment{ .arena = allocator, .fn_scope = true };
    ...
    const function_chunk = function.chunk orelse return error.TestUnexpectedResult;
    try std.testing.expectEqualSlices(u32, &.{1}, function_chunk.default_parameter_indices);
    const input = env.get("propertyInput") orelse return error.TestUnexpectedResult;
    const arguments = [_]Value{input};
    ...
    const saved_this = Value.num(121);
    const saved_new_target = Value.num(122);
    var failures: usize = 0;
    var recovered = false;
    defer machine.arena = allocator;
    for (0..32) |fail_index| {
        ...
        machine.arena = allocator;
        if (result) |value_out| {
            try std.testing.expectEqualStrings("parameter-property", value_out.asStr());
            recovered = true;
        } else |err| {
            try std.testing.expectEqual(error.OutOfMemory, err);
            failures += 1;
...
test "vm: parameter construction allocation failure restores the caller activation" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    var parser = try Parser.init(
        allocator,
        "function Box(value) { this.value = value + value; } function construct(Ctor, input, value = new Ctor(input)) { return value.value; } construct",
    );
    const program = try parser.parseProgram();
    const root = try Compiler.compileProgram(allocator, program);
    var env = Environment{ .arena = allocator, .fn_scope = true };
    ...
    const function_chunk = function.chunk orelse return error.TestUnexpectedResult;
    try std.testing.expectEqualSlices(u32, &.{2}, function_chunk.default_parameter_indices);
    const constructor = env.get("Box") orelse return error.TestUnexpectedResult;
    const input = try Value.strAlloc(allocator, "parameter-construction");
    const arguments = [_]Value{ constructor, input };
    ...
    const saved_this = Value.num(141);
    const saved_new_target = Value.num(142);
    var failures: usize = 0;
    var recovered = false;
    defer machine.arena = allocator;
    for (0..48) |fail_index| {
        ...
        machine.arena = allocator;
        if (result) |value_out| {
            try std.testing.expectEqualStrings("parameter-constructionparameter-construction", value_out.asStr());
            recovered = true;
        } else |err| {
            try std.testing.expectEqual(error.OutOfMemory, err);
            failures += 1;
...
test "vm: super deletion caches native rejection and preserves exact fallback" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    var parser = try Parser.init(allocator, "class Derived extends Base { method(){ return delete super.value; } }");
    const program = try parser.parseProgram();
    const declaration = program.program[0];
    if (declaration.* != .var_decl or declaration.var_decl.init == null)
        return error.TestUnexpectedResult;
