...
inline fn hasObjectBacking(flags: value.ObjectBackingFlags) bool {
    return flags.storage_state or
        ...
        flags.intl_display_names or
        flags.intl_relative_time_format or
        flags.intl_list_format or
        flags.intl_segmenter or
...
test "realm error intrinsics trace and relocate every private identity" {
    const count = interp.ErrorIntrinsics.names.len;
    var old_objects: [count * 2]Object = undefined;
    var new_objects: [count * 2]Object = undefined;
    var intrinsics = interp.ErrorIntrinsics{};
    for (0..count) |index| {
...
test "Environment relocation rewrites every managed binding slot" {
    ...
    defer environment.finalizeOwnedBindingStorage();
    try environment.put("binding", Value.obj(&old_objects[0]));
    const binding_context = environment.bindingHashContext().?;
    try std.testing.expectEqual(&old_objects[0], environment.vars.get(binding_context, "binding").?.asObj());
    try environment.disposables.append(std.testing.allocator, .{
        .value = Value.obj(&old_objects[1]),
        .method = Value.obj(&old_objects[2]),
        .is_async = true,
    });
    try environment.putAlias("imported", &old_environments[1], "exported");
    environment.direct_eval_forward_target = &old_environments[1];
    environment.gc_managed = true;
    ...
    try std.testing.expectEqual(binding_context.seed, environment.bindingHashContext().?.seed);
    try std.testing.expectEqual(&new_objects[0], environment.vars.get(binding_context, "binding").?.asObj());
    ...
    const alias = environment.aliases.get(environment.bindingHashContext(), "imported").?;
...
fn finalizeObjectBacking(o: *Object, a: std.mem.Allocator) usize {
    ...
    if (flags.intl_number_format) {
        if (o.intlNumberFormatData()) |data| {
            data.deinit(a);
            a.destroy(data);
            o.clearIntlNumberFormatData();
        }
        released += 1;
    }
    if (flags.intl_date_time_format) {
        if (o.intlDateTimeFormatData()) |data| {
            data.deinit(a);
            a.destroy(data);
    ...
    if (flags.intl_display_names) {
        if (o.intlDisplayNamesData()) |data| {
            data.deinit(a);
            a.destroy(data);
            o.clearIntlDisplayNamesData();
        }
        released += 1;
    }
    if (flags.intl_relative_time_format) {
        if (o.intlRelativeTimeFormatData()) |data| {
            data.deinit(a);
            a.destroy(data);
            o.clearIntlRelativeTimeFormatData();
        }
        released += 1;
    }
    if (flags.intl_list_format) {
        if (o.intlListFormatData()) |data| {
            data.deinit(a);
            a.destroy(data);
            o.clearIntlListFormatData();
        }
        released += 1;
    }
    if (flags.intl_segmenter) {
        if (o.intlSegmenterData()) |data| {
            data.deinit(a);
            a.destroy(data);
            o.clearIntlSegmenterData();
        }
        released += 1;
    }
    if (flags.intl_plural_rules) {
        if (o.intlPluralRulesData()) |data| {
            data.deinit(a);
            a.destroy(data);
            o.clearIntlPluralRulesData();
        }
        released += 1;
    }
    if (flags.intl_duration_format) {
        if (o.intlDurationFormatData()) |data| {
            data.deinit(a);
            a.destroy(data);
            o.clearIntlDurationFormatData();
        }
        released += 1;
    }
...
test "realm root relocation rewrites active interpreter containers" {
    ...
    var template_node: ast.Node = .undefined_lit;
    try machine.template_cache.put(machine.arena, &machine, @intFromPtr(&template_node), Value.obj(&old_objects[28]));
