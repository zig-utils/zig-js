...
pub fn build(b: *std.Build) void {
    ...
    if (target.result.os.tag == .macos) {
        const run_lldb_jit_registration_test = b.addSystemCommand(&.{
            ...
            "breakpoint delete 1",
            "-o",
            "breakpoint set --name zig_js_baseline_1_observedBaseline",
            "-o",
            "continue",
            "-o",
            "script frame = lldb.debugger.GetSelectedTarget().GetProcess().GetSelectedThread().GetFrameAtIndex(0); assert frame.GetFunctionName() == 'zig_js_baseline_1_observedBaseline'; address = frame.GetPCAddress(); assert address.GetSection().GetName() == '__text'; assert address.GetOffset() == 0",
            "-o",
            "script frame = lldb.debugger.GetSelectedTarget().GetProcess().GetSelectedThread().GetFrameAtIndex(0); target = lldb.debugger.GetSelectedTarget(); module = frame.GetModule(); assert module.GetNumCompileUnits() == 1; unit = module.GetCompileUnitAtIndex(0); entries = [unit.GetLineEntryAtIndex(i) for i in range(unit.GetNumLineEntries()) if unit.GetLineEntryAtIndex(i).IsValid()]; assert entries; assert all(entry.GetFileSpec().GetFilename() == 'native-observability-fixture.js' for entry in entries); assert any(entry.GetLine() == 23 and entry.GetColumn() > 0 for entry in entries); assert min(entry.GetStartAddress().GetLoadAddress(target) for entry in entries) > frame.GetPCAddress().GetLoadAddress(target)",
            "-o",
            "breakpoint set --address '$pc + 8'",
            "-o",
            "continue",
            "-o",
            "script thread = lldb.debugger.GetSelectedTarget().GetProcess().GetSelectedThread(); assert thread.GetNumFrames() > 1; caller = thread.GetFrameAtIndex(1); assert caller.IsValid(); assert caller.GetModule().GetFileSpec().GetFilename() == 'native-observability-lldb'",
            "-o",
            "breakpoint set --file native-observability-fixture.js --line 23",
            "-o",
            "breakpoint delete 2 3",
            "-o",
            "continue",
            "-o",
            "script frame = lldb.debugger.GetSelectedTarget().GetProcess().GetSelectedThread().GetFrameAtIndex(0); line = frame.GetLineEntry(); assert line.IsValid(); assert line.GetFileSpec().GetFilename() == 'native-observability-fixture.js'; assert line.GetLine() == 23; assert line.GetColumn() > 0",
            "-o",
            "breakpoint delete 4",
            "-o",
            "breakpoint set --name zig_js_optimizer_2_observedOptimizer",
            "-o",
            "continue",
            "-o",
            "script frame = lldb.debugger.GetSelectedTarget().GetProcess().GetSelectedThread().GetFrameAtIndex(0); assert frame.GetFunctionName() == 'zig_js_optimizer_2_observedOptimizer'; target = lldb.debugger.GetSelectedTarget(); module = frame.GetModule(); assert module.GetNumCompileUnits() == 1; unit = module.GetCompileUnitAtIndex(0); entries = [unit.GetLineEntryAtIndex(i) for i in range(unit.GetNumLineEntries()) if unit.GetLineEntryAtIndex(i).IsValid()]; assert entries; assert all(entry.GetFileSpec().GetFilename() == 'native-observability-fixture.js' for entry in entries); assert any(entry.GetLine() == 27 and entry.GetColumn() > 0 for entry in entries); assert min(entry.GetStartAddress().GetLoadAddress(target) for entry in entries) > frame.GetPCAddress().GetLoadAddress(target)",
            "-o",
            "breakpoint set --file native-observability-fixture.js --line 27",
            "-o",
            "breakpoint delete 5",
            "-o",
            "continue",
            "-o",
            "script frame = lldb.debugger.GetSelectedTarget().GetProcess().GetSelectedThread().GetFrameAtIndex(0); line = frame.GetLineEntry(); assert frame.GetFunctionName() == 'zig_js_optimizer_2_observedOptimizer'; assert line.IsValid(); assert line.GetFileSpec().GetFilename() == 'native-observability-fixture.js'; assert line.GetLine() == 27; assert line.GetColumn() > 0",
            "-o",
            "breakpoint delete 6",
            "-o",
            ...
            "continue",
            "-o",
            "script frame = lldb.debugger.GetSelectedTarget().GetProcess().GetSelectedThread().GetFrameAtIndex(0); assert frame.GetFunctionName().endswith('native_observability_after_unregister'); target = lldb.debugger.GetSelectedTarget(); assert target.FindFunctions('zig_js_baseline_1_observedBaseline').GetSize() == 0; assert target.FindFunctions('zig_js_optimizer_2_observedOptimizer').GetSize() == 0",
            "-o",
            "continue",
            "--",
        });
    ...
    if (independent_suite_checkout) |checkout| {
        if (independent_suite_zig_js_revision) |revision| {
            if (b.option([]const u8, "independent-suite-recognizer-out", "Absolute recognizer artifact path outside the zig-js worktree")) |output| {
                const recognizer = b.addSystemCommand(&.{ "/usr/bin/env", home_tool, "run", "tools/independent-suite-recognizer.ts" });
                recognizer.addArtifactArg(independent_suite_zig_js);
                recognizer.addArgs(&.{
                    checkout,
                    revision,
                    output,
                ...
                recognizer.step.dependOn(&run_independent_suite_audit.step);
                independent_suite_recognizer_step.dependOn(&recognizer.step);
            } else {
                const missing_output = b.addFail("independent-suite-recognizer requires -Dindependent-suite-recognizer-out=<absolute-path-outside-zig-js>");
                independent_suite_recognizer_step.dependOn(&missing_output.step);
            }
        } else {
            const missing_revision = b.addFail("independent-suite-recognizer requires -Dindependent-suite-zig-js-revision=<40-hex-commit>");
            independent_suite_recognizer_step.dependOn(&missing_revision.step);
        }
    } else {
        const missing_checkout = b.addFail("independent-suite-recognizer requires -Dindependent-suite-checkout=<verified-out-of-tree-path>");
        independent_suite_recognizer_step.dependOn(&missing_checkout.step);
    }
    ...
    if (target.result.os.tag == .macos) {
        ...
        if (b.option([]const u8, "instrumentation-overhead-profile", "Instrumentation profile: execution_attribution or native_observability")) |profile|
            run_instrumentation_overhead.addArgs(&.{ "--instrumentation", profile });
