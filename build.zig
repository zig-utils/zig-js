...
pub fn build(b: *std.Build) void {
    ...
    const fuzz_midgc = b.option(bool, "fuzz-midgc", "threadfuzz: mid-script parallel GC wait-pump, microtask churn, creator-owned buffers, sync-wait cleanup including property waitAsync timeout/live tickets, teardown-termination, promise-publication, script/module Worker/SAB cleanup, Worker exception cleanup, Worker close/terminate drain/drop, and weak-collection cleanup profile") orelse false;
    const fuzz_lifecycle = b.option(bool, "fuzz-lifecycle", "threadfuzz: deterministic termination, pending reaction teardown, Worker/module-worker graph overlap, Worker/module-worker exception/finalization cleanup and terminate/thread teardown, Worker/thread/finalization scheduling, asyncHold barging, Promise microtask churn, creator-owned buffer and Worker clone lifetime, Thread.restrict/ThreadLocal isolation, ThreadLocal finalization cleanup, waitAsync/finalization cleanup, finalization cleanup/waiter/unregister, and exact script/module Worker close/terminate drain/drop lifecycle profile") orelse false;
