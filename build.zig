...
pub fn build(b: *std.Build) void {
    ...
    if (target.result.os.tag == .macos) {
        const run_lldb_jit_registration_test = b.addSystemCommand(&.{
            ...
            "continue",
            "-o",
            "script frame = lldb.debugger.GetSelectedTarget().GetProcess().GetSelectedThread().GetFrameAtIndex(0); assert frame.GetFunctionName().endswith('native_observability_after_unregister'); target = lldb.debugger.GetSelectedTarget(); assert target.FindFunctions('zig_js_baseline_1_observedNative').GetSize() == 0",
            "-o",
            "continue",
            "--",
        });
