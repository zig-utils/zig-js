...
pub fn build(b: *std.Build) void {
    ...
    const fuzz_midgc = b.option(bool, "fuzz-midgc", "threadfuzz: mid-script parallel GC wait-pump, microtask churn, finalization asyncJoin/unregister cleanup, waitAsync/finalization cleanup, Condition.asyncWait/finalization cleanup, late asyncJoin fulfillment/rejection cleanup, creator-owned buffers plus script/module Worker clone/finalization cleanup, nested Thread asyncJoin cleanup, ThreadLocal lifecycle/finalization/termination cleanup and Thread.restrict lifecycle/finalization, sync-wait cleanup including property waitAsync timeout/live tickets plus retained-SAB Worker overlap and same-primitive burst release, sync timeout exit, Atomics.Mutex.lockIfAvailable acquire/timeout cleanup, Atomics.Condition.wait notify/reacquire cleanup, asyncHold release/waiter cleanup, teardown-termination, promise-publication, script/module Worker/SAB cleanup, Worker exception cleanup, Worker close/terminate drain/drop, script/module Worker terminate/finalization cleanup, script/module Worker Condition.asyncWait teardown cleanup, script/module Worker ThreadLocal/asyncHold teardown cleanup, and weak-collection cleanup profile") orelse false;
    const fuzz_lifecycle = b.option(bool, "fuzz-lifecycle", "threadfuzz: deterministic resizable ArrayBuffer/DataView no-GIL races, termination, pending reaction teardown, Worker/module-worker graph overlap, Worker/module-worker exception/finalization cleanup and script/module terminate/finalization plus terminate/thread teardown including script/module condition async plus script/module ThreadLocal/asyncHold/waiter teardown, ThreadLocal cleanup, ThreadLocal plus asyncHold release/waiter cleanup, and script/module waitAsync cleanup, Atomics.Mutex/Condition token waits plus lockIfAvailable acquire/timeout paths, Worker/thread/finalization scheduling, asyncHold/Condition.asyncWait barging and cleanup, asyncHold throw/release/waiter cleanup, Promise microtask churn, late asyncJoin fulfillment/rejection cleanup, creator-owned buffer and script/module Worker clone/finalization lifetime, nested Thread asyncJoin cleanup, Thread.restrict/ThreadLocal isolation, Thread.restrict/ThreadLocal finalization cleanup, waitAsync/finalization cleanup, finalization cleanup/waiter/unregister, and exact script/module Worker close/terminate drain/drop lifecycle profile") orelse false;
