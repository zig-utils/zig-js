...
pub fn build(b: *std.Build) void {
    ...
    const fuzz_midgc = b.option(bool, "fuzz-midgc", "threadfuzz: mid-script parallel GC wait-pump, microtask churn, late asyncJoin cleanup, creator-owned buffers, nested Thread asyncJoin cleanup, ThreadLocal and Thread.restrict finalization, sync-wait cleanup including property waitAsync timeout/live tickets plus retained-SAB Worker overlap and same-primitive burst release, sync timeout exit, Atomics.Mutex.lockIfAvailable acquire/timeout cleanup, Atomics.Condition.wait notify/reacquire cleanup, asyncHold release/waiter cleanup, teardown-termination, promise-publication, script/module Worker/SAB cleanup, Worker exception cleanup, Worker close/terminate drain/drop, and weak-collection cleanup profile") orelse false;
    const fuzz_lifecycle = b.option(bool, "fuzz-lifecycle", "threadfuzz: deterministic termination, pending reaction teardown, Worker/module-worker graph overlap, Worker/module-worker exception/finalization cleanup and terminate/thread teardown including condition async and script/module waitAsync cleanup, Atomics.Mutex/Condition token waits plus lockIfAvailable acquire/timeout paths, Worker/thread/finalization scheduling, asyncHold/Condition.asyncWait barging and cleanup, asyncHold throw/release/waiter cleanup, Promise microtask churn, late asyncJoin observer cleanup, creator-owned buffer and script/module Worker clone/finalization lifetime, nested Thread asyncJoin cleanup, Thread.restrict/ThreadLocal isolation, Thread.restrict/ThreadLocal finalization cleanup, waitAsync/finalization cleanup, finalization cleanup/waiter/unregister, and exact script/module Worker close/terminate drain/drop lifecycle profile") orelse false;
