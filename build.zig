...
pub fn build(b: *std.Build) void {
    ...
    const fuzz_midgc = b.option(bool, "fuzz-midgc", "threadfuzz: mid-script parallel GC wait-pump, microtask churn, late asyncJoin fulfillment/rejection cleanup, creator-owned buffers, nested Thread asyncJoin cleanup, ThreadLocal finalization/termination cleanup and Thread.restrict finalization, sync-wait cleanup including property waitAsync timeout/live tickets plus retained-SAB Worker overlap and same-primitive burst release, sync timeout exit, Atomics.Mutex.lockIfAvailable acquire/timeout cleanup, Atomics.Condition.wait notify/reacquire cleanup, asyncHold release/waiter cleanup, teardown-termination, promise-publication, script/module Worker/SAB cleanup, Worker exception cleanup, Worker close/terminate drain/drop, script/module Worker Condition.asyncWait teardown cleanup, script/module Worker ThreadLocal/asyncHold teardown cleanup, and weak-collection cleanup profile") orelse false;
