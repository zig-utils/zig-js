...
pub fn build(b: *std.Build) void {
    ...
    const fuzz_lifecycle = b.option(bool, "fuzz-lifecycle", "threadfuzz: deterministic termination, pending reaction teardown, Worker/module-worker graph overlap, Worker/thread/finalization scheduling, Worker terminate/thread teardown, asyncHold barging, Promise microtask churn, creator-owned buffer lifetime, Thread.restrict/ThreadLocal isolation, ThreadLocal finalization cleanup, waitAsync/finalization cleanup, finalization cleanup/waiter/unregister, and exact Worker close/terminate drain/drop lifecycle profile") orelse false;
