...
pub fn build(b: *std.Build) void {
    ...
    const fuzz_midgc = b.option(bool, "fuzz-midgc", "threadfuzz: mid-script parallel GC wait-pump, microtask churn, creator-owned buffers, nested Thread asyncJoin cleanup, ThreadLocal and Thread.restrict finalization, sync-wait cleanup including property waitAsync timeout/live tickets and same-primitive burst release, sync timeout exit, Atomics.Mutex.lockIfAvailable acquire/timeout cleanup, Atomics.Condition.wait notify/reacquire cleanup, asyncHold release/waiter cleanup, teardown-termination, promise-publication, script/module Worker/SAB cleanup, Worker exception cleanup, Worker close/terminate drain/drop, and weak-collection cleanup profile") orelse false;
