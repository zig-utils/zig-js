...
The remaining lexical boundary is per-iteration identity: a loop-head or loop-body lexical captured by a closure stays on the tree-walker because each iteration needs a fresh binding cell rather than another name-to-slot mapping. Deferred class members are admitted when they use only globals, but a member that captures one of these frame slots rejects its enclosing function. Base constructors without fields can use bytecode; derived constructors and constructors with field initialization retain an explicit tree-walker barrier until bytecode activations model their initialization state. Per-iteration environments are the next scope-model step.
