...
| --- | --- | --- |
| ECMAScript conformance | `docs/.data/test262.json` | `bun scripts/gen-test262-data.ts --from run.txt` |
| Documentation output | `docs-site-manifest-v1.json` | `zig build docs-manifest-update` |
| WebAssembly conformance | per-profile inventories + `wasm-conformance-matrix.json` | `tools/wasm-conformance-matrix.py` |
...
8. **Docs** — `tools/docs-link-check.py` resolves every internal link and
   configured navigation entry, then `zig build docs-build` renders all 49
   public pages and checks every output hash against the checked manifest. The
   builder, preview server, component expansion, highlighting, search index,
   sitemap, and asset copier are all in-tree Zig and require no registry install.
