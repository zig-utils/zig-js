...
Some functions currently accept JavaScriptCore-shaped signatures but do not yet honor every argument: `JSEvaluateScript` ignores `thisObject` / `sourceURL` / `startingLineNumber`. Treat those as pre-stabilization gaps, not long-term ABI commitments: call sites compile today, but inert parameters should be implemented, renamed, or removed before the surface is declared stable. `JSGlobalContextRetain` / `JSGlobalContextRelease` maintain a real C-API reference count, `JSObjectMakeFunctionWithCallback` honors the provided function name, and `JSObjectSetProperty` honors property attributes.
