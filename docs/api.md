...
or extension totals. The pinned Home inventory currently reports eighty-one
private exports and 350 pending private symbols; `zig build test-home-private-abi` and
`zig build test-private-jstype` are their focused compile-link-runtime gates.
The eighty-one cover JSC64 identity, cell equality,
...
pinned messages and legacy codes. The combined 79-symbol fixture also
...
of which the same seventy-four shims are implemented and 347 remain pending. Its
