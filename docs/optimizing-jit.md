...
The first AArch64 loop OSR region handles a guarded numeric header plus one straight-line numeric body. After a hot backedge, native code imports the exact header frame, executes and accounts for either the exit header or one complete body iteration, then reconstructs the exit or next-header edge without replay. SSA block arguments are copied into body scratch slots explicitly, and the updated loop-carried values are recovered at the next header. Each native entry remains bounded to one iteration; a budget or 1,024-step checkpoint crossing stays in bytecode. Multi-iteration native backedges and internal polls remain tracked by #435.

Executable-code invalidation now closes native entry immediately while existing owner leases keep published mappings alive. Function entry, direct calls, and every loop OSR backedge poll the owner before entering native code. Optimizer machine code then compares the artifact's owner generation again before its first operation, closing the race between the VM poll and native entry. Either rejection performs no step accounting or state mutation and continues in bytecode. Long-running native regions still need resumable internal invalidation polls before #432's invalidation acceptance item can close.
