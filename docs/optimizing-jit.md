...
Deoptimization metadata can also own ordered catch/finally records with exact stack depths. Reconstruction validates every range and handler before reserving both VM lists, then replaces locals, operand stack, accumulator, handlers, and IP as one no-fail commit; malformed metadata leaves the activation untouched. The optimizer graph propagates structured normal-flow `push_handler`/`pop_handler` stacks through frame and edge states and rejects mismatched merges or unbalanced pops. An explicit throw to a catch now publishes an exceptional SSA edge carrying the unwound locals, operand stack, exception value, and outer handler stack. A deterministic chain—including nested catches—can execute through the catch and return entirely in AArch64 native code; uncaught throws retain their exact side exit. Return/break/continue through `finally`, every call/construct shape, and interpreter-owned environment, allocation, object-initialization, iteration, property, coercion, brand, and scope effects remain terminal optimizer exits before observable work. Debugger, host, or profiler statement hooks force bytecode across function entry, loop OSR, and direct-call paths even if an artifact and execution lease already exists. Native finally dispatch and exceptional successors from calls/effects remain tracked by #436.
