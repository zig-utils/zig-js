...
The first AArch64 loop OSR region handles a guarded numeric header, an owned chain of conditional exits, an owned chain of conditional continue/backedge guards, and then an owned sequence of branch segments. Every early latch owns its edge copies, exact bytecode charge, checkpoint/budget accounting, moving safepoint, and backedge. Shared joins may feed the next branch directly or through one linear merge; the terminal segment either merges into one latch or lets both arms backedge independently. Empty guard and branch sequences cover the straight-line shape. A function with multiple loop headers publishes only the exact last bytecode header, which is the innermost header for ordinary structured nesting; unmatched outer backedges stay in bytecode. Region discovery follows only the selected loop, so unrelated branches after its exit do not reject compilation. The header reserves the longest recognized path, while generated code charges only the path taken. An exit reconstructs its exact edge state and resumes the outer target without executing it twice. Native code imports the exact header frame, applies block arguments as raw parallel assignments, and executes multiple iterations before reconstructing an exit edge. Only values consumed by numeric operations require Number guards; recovery-only object locals remain precisely mapped. The lowering breaks copy cycles with one reusable scratch slot, and swapped multi-local backedges verify exact recovery. Every native header polls invalidation and side-exits exact current state before pending work. A deterministic instrumented race changes the owner generation only after a completed native backedge and verifies exact header reconstruction on the next poll under normal and TSan gates; production artifacts emit no observer. Irreducible two-entry loops reject before publication. Fused nested-loop regions, arbitrary reconvergence, and fully general reducible regions remain outside this backend.
