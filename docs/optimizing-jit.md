...
The first AArch64 loop OSR region handles a guarded numeric header with either a straight-line body or one inner `if/else` diamond. Each arm owns its exact bytecode cost; the header reserves the larger safe quantum, then the selected arm updates steps, budget, and checkpoint distance by its actual cost before the backedge. Native code imports the exact header frame, applies block arguments as parallel assignments, and executes multiple iterations before reconstructing the exit edge. The lowering breaks copy cycles with one reusable scratch slot; swapped multi-local backedges verify exact recovery. Every native header polls invalidation and side-exits exact current state before pending work. A deterministic instrumented race changes the owner generation only after a completed native backedge and verifies exact header reconstruction on the next poll under normal and TSan gates; production artifacts emit no observer. Deeper control, multiple latches/exits, and irreducible graphs remain outside this region.
