...
Executable-code invalidation closes affected native entry immediately and rotates the owner's execution epoch. Every published artifact owns an independent invalidation cell; the owner-wide generation remains an attribution counter rather than the guard for all native code. A Class-A mutation first uses the watched-shape bit filter as a cheap over-approximating precheck, then scans immutable artifact property-cache metadata under the owner lock and retires only mappings that name the exact pre-mutation shape. Surviving mappings remain published and enterable, and the watched-shape filter is rebuilt from their assumptions. An unknown shape, a shapeless receiver, or allocation failure in the precise retirement path fails closed through the full-owner stop.

Retired code and all artifact-owned property caches, call links, handler chains, recovery records, and stack maps move into the detached epoch. Its final reader reclaims them without waiting for readers that began in a later generation. When precise retirement leaves survivor mappings reachable across an epoch rotation, reclamation retains the cross-epoch older-reader frontier until every potentially older lease is quiescent. Live, retired, precisely retired, surviving, and cumulatively reclaimed artifact counts and executable bytes are maintained at every transition. Compilation claims use a separate lease, so invalidation rejects a stale publisher before rotation while long-running bytecode does not pin unrelated generations. Native entry carries both its artifact cell and outer epoch token and checks them around artifact loads, preventing an invalidated execution from entering either retired or newer code.

Mutation classification follows the actual assumption boundary. Existing data-property value stores and representation-preserving `defineProperty` calls keep the same guarded shape/slot contract and preserve native code. Ordinary deletion, attribute or accessor redefinition, prototype replacement, and shape transitions pass the exact pre-mutation shape into precise retirement; a transition whose old shape is absent from every artifact retires nothing. Failed non-configurable deletion and empty descriptors likewise preserve artifacts. The mutator never waits on the heap-owned JIT/GC conductor while holding the indexed-name exclusion, and remains a cooperative-GC participant while it waits, so an active collection can complete before retirement retries. Sibling precise realms share the same conductor and executable owner. The race matrix, optimizer-off/on property differential, and focused retirement evidence are tracked on [#471](https://github.com/zig-utils/zig-js/issues/471).
