...
The first executable operation is `to_numeric` on deterministic AArch64 paths. It is an effectful SSA value with a normal successor; `dup` remains a zero-cost alias, so real postfix-update bytecode can consume the native result before a later side exit. Generated code publishes the operation id, origin, deopt index, and exact step delta, calls the installed VM dispatcher, stores a successful result, and branches separately for an uncaught throw, owned exceptional continuation, allocation failure, invalidation, or fail-closed operation trap. The VM validates the descriptor id, input range, opcode, origin, deopt index, target index, and executable step delta before coercing.
