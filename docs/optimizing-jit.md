...
The executable operations are `to_numeric`, ordinary and explicit-`this` calls, and construction on deterministic AArch64 paths. Each is an effectful SSA value with a normal successor; `dup` remains a zero-cost alias, so real postfix-update bytecode can consume a coercion result before a later side exit. Invocation nodes stage their callee/constructor, optional receiver, and dynamic argument vector in contiguous scratch slots, execute once with the opcode-specific VM semantics, and store the returned value for the native suffix. Generated code publishes the operation id, origin, deopt index, and exact step delta, then branches separately for an uncaught throw, owned exceptional continuation, allocation failure, invalidation, or fail-closed operation trap. The VM validates the descriptor id, input range and arity, opcode, origin, deopt index, target index, and executable step delta before dispatch.
...
The callback stack map marks every live scratch recovery value plus the descriptor's explicit inputs; frame recoveries keep their existing masks. Native entry registers that map before generated code runs, so object inputs remain movable and traceable while coercion hooks, callees, or constructors re-enter JavaScript. Focused witnesses cover ordinary/explicit-`this` return, constructor return and one-shot throw, dynamic arity and roots, catch/finally continuation, primitive and object coercion, malformed descriptors and targets, OOM, trap, invalidation, unrelated fallback, and exact step ownership. Existing frame offsets are unchanged, so numeric artifacts without runtime operations remain unmanaged leaves.
