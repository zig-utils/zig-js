...
The AArch64 loop tier keeps specialized encodings for common straight, guard-chain, and sequential-diamond shapes, then falls back to a general single-header region. Removing edges back to the selected header must leave a single-entry DAG; the lowerer rejects second entries and internal cycles before publication. It topologically owns every block and gives every CFG edge a distinct parallel-copy operation group, so arbitrary nested branches may reconverge, exit, or reach independent latches without conflating SSA state. The header reserves the longest path, while generated code charges each executed block incrementally against exact steps, checkpoint distance, and budget. Every latch applies its own header copies, publishes a moving safepoint, and backedges; every exit reconstructs its exact source edge and resumes the outer target without double execution.

A function with multiple loop headers publishes only the exact last bytecode header, which is the innermost header for ordinary structured nesting; unmatched outer backedges stay in bytecode. Native code imports that exact frame and executes multiple iterations before reconstructing an exit. Only values consumed by numeric operations require Number guards; recovery-only object locals remain precisely mapped. Parallel-copy cycles use one reusable scratch slot, and swapped multi-local backedges verify exact recovery. Every native header polls invalidation before pending work. A deterministic instrumented race changes the owner generation only after a completed native backedge and verifies exact reconstruction on the next poll under normal and TSan gates; production artifacts emit no observer. Irreducible two-entry loops reject before publication. Cyclic/fused nested-loop regions remain outside this backend.
