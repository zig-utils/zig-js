...
The executable operations are `to_numeric`, `get_prop`, `get_index`, `set_prop`, `set_index`, `in`, `instanceof`, private-brand `in`, ordinary and explicit-`this` calls, and construction on deterministic AArch64 paths. Each is an effectful SSA value with a normal successor. Invocation and binary operations stage contiguous dynamic inputs; named operations use artifact-owned property/private names, never borrowed chunk storage. Writes call the canonical VM mutation path, so setters, proxy traps, assignment results, and GC barriers are preserved; both forms return the right-hand side. Membership operations return Boolean SSA values while retaining canonical key coercion, proxy `has`, `Symbol.hasInstance`, prototype, private-brand, and TypeError behavior. The callback executes once and stores its result for the native suffix. Generated code publishes the operation id, origin, deopt index, and exact step delta, then branches separately for an uncaught throw, owned exceptional continuation, allocation failure, invalidation, or fail-closed operation trap. The VM validates the descriptor id, payload, input range and arity, opcode, origin, deopt index, target index, and executable step delta before dispatch.
...
The callback stack map marks every live scratch recovery value plus the descriptor's explicit inputs; frame recoveries keep their existing masks. Native entry registers that map before generated code runs, so receivers, keys, assigned values, membership operands, callees, and constructors remain movable and traceable while accessors, proxy traps, coercion, or calls re-enter JavaScript. Focused witnesses cover complete method chains, read/write ordering, assignment results, `in` proxy traps, custom `Symbol.hasInstance`, private brands, one-shot observable effects and throws, dynamic arity and roots, catch/finally continuation, malformed descriptors and targets, OOM, trap, invalidation, unrelated fallback, and exact step ownership. Existing frame offsets are unchanged, so numeric artifacts without runtime operations remain unmanaged leaves.
