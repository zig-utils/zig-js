...
The first AArch64 loop OSR region handles a guarded numeric header, an owned chain of conditional exits, and then an owned sequence of branch segments. Shared joins may feed the next branch directly or through one linear merge; the terminal segment either merges into one latch or lets both arms backedge independently. Empty exit and branch sequences cover the straight-line shape. Exit discovery follows only the loop region, so unrelated branches after the outer exit do not reject compilation. The header reserves the sum of each segment's maximum cost, while generated code increments steps and checkpoint/budget distances by only the selected arm's incremental cost. An exit reconstructs its exact edge state and resumes the outer target without executing it twice. Native code imports the exact header frame, applies block arguments as raw parallel assignments, and executes multiple iterations before reconstructing an exit edge. Only values consumed by numeric operations require Number guards; recovery-only object locals remain precisely mapped. The lowering breaks copy cycles with one reusable scratch slot, and swapped multi-local backedges verify exact recovery. Every native header polls invalidation and side-exits exact current state before pending work. A deterministic instrumented race changes the owner generation only after a completed native backedge and verifies exact header reconstruction on the next poll under normal and TSan gates; production artifacts emit no observer. Irreducible two-entry loops reject before publication. Nested loops, arbitrary reconvergence, and more than two latches remain outside this backend.
