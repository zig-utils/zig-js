...
Executable-code invalidation closes native entry immediately, jettisons every registered tier, and rotates the owner's execution epoch. Published code and all artifact-owned property caches, call links, handler chains, recovery records, and stack maps move into the retired epoch; its final reader reclaims them without waiting for readers that began in a later generation. Live, retired, and cumulatively reclaimed artifact counts and executable bytes are maintained at every transition. Compilation claims use a separate lease, so invalidation rejects a stale publisher before rotation while long-running bytecode does not pin unrelated generations. Native entry carries the outer epoch token and checks it around artifact loads, preventing an invalidated execution from entering newer code. VM and canonical named-property mutations fire Class-A invalidation once a parallel realm has published native heap assumptions, scoped to the shapes that code actually speculates on: the owner summarizes the shape tokens of its published property caches in a bit filter, and a mutation whose receiver shape is absent from it skips the stop. The filter only over-approximates — a hash collision, an unknown shape, a shapeless receiver, and any site with no object in hand all keep the unconditional behavior — so no shape an artifact guards on can be missed. A mutator waiting for the heap-owned JIT/GC conductor remains a cooperative-GC participant, so it can acknowledge the active request before retrying the generation bump. Sibling precise realms share the same conductor and executable owner.

The measured cost of that design is a Class-A stop being all-or-nothing once it fires. `cve/mc-val-fire-vs-link.js` reports `publications=271244 invalidations=271244` under no-GIL against `publications=40 invalidations=0` serialized — one jettison per compile where forty compiles is what the workload needs, because a foreign thread adding properties in a loop discards every tier the hot thread publishes. Two things have to change together to fix it, and neither alone is sufficient:

- **Scope.** `Owner.clear` jettisons every registered tier, not the artifacts speculating on the mutated shape. The firing site already computes the pre-store shape token for the filter check and could pass it through.
- **Generation.** Scoping the jettison alone gains nothing, because `invalidation_generation` is one owner-wide atomic that every published artifact points at. Bumping it closes native entry for *all* code whether or not its tier was retired, so surviving artifacts still stop. Per-artifact invalidation therefore needs per-artifact generations, which is a change to the invalidation protocol rather than to `clear`.

There is also a policy question underneath both. A property **add** transitions the receiver to a new shape, and generated guards revalidate the live shape before touching storage, so the guard already fails on its own — the stop buys nothing there. What genuinely needs a stop is an in-place change that keeps the shape while breaking an assumption: deletion, an attribute or accessor redefinition, a prototype replacement. Narrowing on that distinction would take this workload near zero rather than redistributing the work, but it changes when a concurrent JIT invalidates and needs its own soundness review and full-corpus evidence. Tracked on [#49](https://github.com/zig-utils/zig-js/issues/49).
