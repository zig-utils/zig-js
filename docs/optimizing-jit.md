...
The executable operations are `to_numeric`, `get_prop`, `get_index`, `set_prop`, `set_index`, ordinary and explicit-`this` calls, and construction on deterministic AArch64 paths. Each is an effectful SSA value with a normal successor. Invocation and computed operations stage contiguous dynamic inputs; named operations use artifact-owned property names, never borrowed chunk storage. Writes call the canonical VM mutation path, so setters, proxy traps, strict assignment results, and GC barriers are preserved; both forms return the right-hand side. Computed writes evaluate the right-hand side before the callback, then reject a nullish base before observable key coercion. The callback executes once and stores its result for the native suffix. Generated code publishes the operation id, origin, deopt index, and exact step delta, then branches separately for an uncaught throw, owned exceptional continuation, allocation failure, invalidation, or fail-closed operation trap. The VM validates the descriptor id, payload, input range and arity, opcode, origin, deopt index, target index, and executable step delta before dispatch.
