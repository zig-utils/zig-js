...
The first executable operation is handler-free `to_numeric` on deterministic AArch64 paths. It is an effectful SSA value with a normal successor; `dup` remains a zero-cost alias, so real postfix-update bytecode can consume the native result before a later side exit. Generated code publishes the operation's origin and deopt index, accounts through the coercion exactly once, calls the installed VM dispatcher, stores a successful result, and branches separately for an uncaught throw, allocation failure, invalidation, or fail-closed operation trap. The VM validates the descriptor id, input range, opcode, origin, deopt index, and executable step delta before coercing. Active-handler coercions retain the exact pre-effect side exit until native exceptional-edge continuation lands.

The callback stack map marks every live scratch recovery value plus the descriptor's explicit inputs; frame recoveries keep their existing masks. Native entry registers that map before generated code runs, so object inputs remain movable and traceable while `valueOf` or another coercion hook re-enters JavaScript. Focused witnesses cover primitive completion, ten successful object conversions followed by an uncaught user throw, malformed descriptors, OOM, trap, invalidation, handler fallback, and exact step ownership. Existing frame offsets are unchanged, so numeric artifacts without runtime operations remain unmanaged leaves.
