...
The published arrays workload still selects the older guarded VM packed-push and packed-sum kernels before optimizer OSR. [#451](https://github.com/zig-utils/zig-js/issues/451) specializes optimizer exact-end assignment, literal append, and one-argument intrinsic `push` only when capacity is already available; it does not change that workload or the accepted benchmark score. Reallocation and wider push forms remain [#449](https://github.com/zig-utils/zig-js/issues/449).
