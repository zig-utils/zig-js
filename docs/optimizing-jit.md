...
Generated loops now reach a moving-GC safepoint at most 64 completed backedges after a compaction request can be observed. The poll runs after canonical backedge copies, publishes the exact loop-header recovery index, preserves the native ABI state, and enters the runtime with no managed pointer held only in a register. Compaction rewrites marked frame and scratch words in place, then native execution reloads them on the next iteration. This poll is separate from the exact 1,024-step budget, termination, trap, and GIL checkpoint, so it cannot invent or delay those semantics. A real-GC test moves a recovery-only object during optimized execution and verifies the post-exit property read under normal and TSan gates. No-GIL relocation stress and pointer-producing native operations remain for #437.
