...
The executable operations are `to_numeric`, `get_prop`, `get_index`, ordinary and explicit-`this` calls, and construction on deterministic AArch64 paths. Each is an effectful SSA value with a normal successor. Invocation and computed-read nodes stage contiguous dynamic inputs; named reads use artifact-owned copies of their property names, never borrowed chunk storage. The callback executes once with opcode-specific VM semantics and stores the returned value for the native suffix. Generated code publishes the operation id, origin, deopt index, and exact step delta, then branches separately for an uncaught throw, owned exceptional continuation, allocation failure, invalidation, or fail-closed operation trap. The VM validates the descriptor id, payload, input range and arity, opcode, origin, deopt index, target index, and executable step delta before dispatch.
