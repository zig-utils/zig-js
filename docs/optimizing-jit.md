...
The published arrays workload still selects the older guarded VM packed-push and packed-sum kernels before optimizer OSR. [#451](https://github.com/zig-utils/zig-js/issues/451) added direct spare-capacity append; [#452](https://github.com/zig-utils/zig-js/issues/452) adds zero/variadic direct publication plus precisely rooted reallocating push. The [exact-parent array profile](.data/optimizer-array-profile-2026-07-22.md) shows no transfer into the new helper, so the measured tier priority and accepted benchmark score remain unchanged.
