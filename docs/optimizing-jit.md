...
The executable operations are `to_numeric`; unary `-`/`+`/`!`/`~`, `typeof`, increment/decrement, ToString, and ToPropertyKey; exponentiation; all bitwise and shift operators; property reads/writes; all three membership forms; ordinary and explicit-`this` calls; and construction on deterministic AArch64 paths. `void` lowers directly to undefined; dynamic forms are effectful SSA values with normal successors. The tree walker, bytecode VM, and optimizer callback share canonical unary and binary semantic paths. Binary operands are rooted together and coerced left-to-right, preserving Number/BigInt results, mixed-type and Symbol errors, and one-shot exceptions. Invocation and multi-input operations stage contiguous dynamic inputs; named operations use artifact-owned property/private names. Writes retain canonical mutation behavior, and membership retains canonical proxy, `Symbol.hasInstance`, prototype, and private-brand behavior. Generated code publishes exact operation/origin/deopt/step state and distinguishes normal, throw, exceptional continuation, allocation failure, invalidation, and fail-closed trap outcomes.
...
The callback stack map marks every live scratch recovery value plus the descriptor's explicit inputs; frame recoveries keep their existing masks. Native entry registers that map before generated code runs, so coercion inputs, receivers, keys, assigned values, membership operands, callees, and constructors remain movable and traceable across re-entry. Focused witnesses cover every primitive unary result, pure `void`, Boolean control after a callback, one-shot conversion throws, complete method chains, mutation/membership effects, dynamic roots, catch/finally continuation, malformed descriptors, OOM, traps, invalidation, fallback, and exact step ownership. Existing frame offsets are unchanged, so numeric artifacts without runtime operations remain unmanaged leaves.
