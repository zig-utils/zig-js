...
The executable operations are `to_numeric`, unary `-`/`+`/`!`/`~`, `typeof`, increment/decrement, ToString, ToPropertyKey, property reads/writes, all three membership forms, ordinary and explicit-`this` calls, and construction on deterministic AArch64 paths. `void` lowers directly to undefined; the dynamic forms are effectful SSA values with normal successors. The tree walker, bytecode VM, and optimizer callback share one unary semantic path, preserving Number/BigInt results, conversion order, and one-shot exceptions. Invocation and multi-input operations stage contiguous dynamic inputs; named operations use artifact-owned property/private names. Writes retain canonical mutation behavior, and membership retains canonical proxy, `Symbol.hasInstance`, prototype, and private-brand behavior. Generated code publishes exact operation/origin/deopt/step state and distinguishes normal, throw, exceptional continuation, allocation failure, invalidation, and fail-closed trap outcomes.
