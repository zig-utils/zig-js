...
The executable operations are `to_numeric` and ordinary `call` on deterministic AArch64 paths. Both are effectful SSA values with normal successors; `dup` remains a zero-cost alias, so real postfix-update bytecode can consume a coercion result before a later side exit. A call stages its callee and dynamic argument vector in contiguous scratch slots, invokes the callee once with `undefined` as `this`, and stores the returned value for the native suffix. Generated code publishes the operation id, origin, deopt index, and exact step delta, then branches separately for an uncaught throw, owned exceptional continuation, allocation failure, invalidation, or fail-closed operation trap. The VM validates the descriptor id, input range and arity, opcode, origin, deopt index, target index, and executable step delta before dispatch.
...
The callback stack map marks every live scratch recovery value plus the descriptor's explicit inputs; frame recoveries keep their existing masks. Native entry registers that map before generated code runs, so object inputs remain movable and traceable while coercion hooks or callees re-enter JavaScript. Focused witnesses cover ordinary call return, dynamic arity and roots, one-shot catch/finally throws, primitive completion, successful and throwing object conversions, malformed descriptors and targets, OOM, trap, invalidation, unrelated coercion fallback, and exact step ownership. Existing frame offsets are unchanged, so numeric artifacts without runtime operations remain unmanaged leaves.
