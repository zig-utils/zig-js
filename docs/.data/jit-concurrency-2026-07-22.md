...
| normal + TSan | Class-A / shared-GC conductor ordering | 3 / 3 each |
...
The conductor witness holds a shared collection window while a competing Class-A invalidator attempts to run. The optimizer generation remains unchanged until the window closes, then advances exactly once before the invalidator returns. Cooperative, abort-safe M3, and allocation-failure collectors retain their existing bounded convergence under the same lock in normal and TSan modes.
