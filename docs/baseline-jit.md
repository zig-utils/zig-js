...
All native emitters share the versioned `js.jit.ExecutableMemoryProfile`
contract. Version 1 owns allocation, instruction-cache synchronization, the
single writable-to-executable publication transition, exact used-byte ranges,
and retirement. Its stable eight-byte encoding contains only owned version,
platform, architecture, backend, cache, protection, and patching identifiers;
it never contains Zig target ordinals, runtime addresses, or process-local
state. Generated bytes are immutable after publication.

The memory contract currently has these target profiles:

- macOS allocates `MAP_JIT` mappings and uses
  `pthread_jit_write_protect_np` around writes, followed by
  `sys_icache_invalidate` on AArch64;
- Linux AArch64 and x86-64 allocate anonymous read/write pages, synchronize the
  instruction cache when the architecture requires it, and make one
  process-wide `mprotect` transition to read/execute;
- every other target fails closed before allocation.

Executable-memory capability is deliberately separate from native-tier
availability. A Linux profile proves the allocation and W^X boundary but does
not advertise a Linux code generator; `js.jit.supported` remains true only for
the released macOS AArch64 emitter, and unsupported tiers continue in bytecode.
This prevents a new allocator backend from silently becoming a public codegen
claim.

`zig build executable-memory-wx-test` independently executes tiny
architecture-specific code after publication and uses child processes to prove
that executing while writable and writing after publication both fault. CI
runs this as its own Linux x86-64 gate. Bytecode analysis remains separate from
the AArch64 assembler, and an x86-64 emitter remains future backend work.
