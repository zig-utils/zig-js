...
- **Thread-safe allocation.** GC cell slabs: `Context.GcCellBacking` recycles
  ...
  cheaper frees/remaps during collection/teardown, skips freelist rebuilds during
  context bulk teardown, and locks internally under no-GIL `parallel_gc`;
...
1. **`evaluate` / `evaluateModule` realm state.** Good news from the audit: the
   ...
   *Abort-safe driver — IMPLEMENTED (`parallel_midscript_gc`), validated
   ...
   - **Publication.** Running peers publish precise roots
     ...
     ThreadLocal maps, thread completion results, and release-function lock
     records are traced or insertion-barriered when populated. Contended
     ...
     both intact. The regression case from the
