...
- Optional per-context allocator pressure cap:
  ...
  abort-safe parallel collector. `Lock.asyncHold` queues, condition waiter
  queues/tickets, ThreadLocal maps, and other trace-sensitive side-store locks
  fail closed while held, because the tracer may need the same lock to find
  hidden JS roots. Mutable generator / iterator-helper side stores
