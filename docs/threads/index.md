...
| Shared-realm `Thread` API | Implemented in `src/jsthread.zig`, `src/gil.zig`, and `src/context.zig`; parallel by default, GIL opt-out available. | PR-249 green coverage: 227 promoted files out of 259 executable files (226 default `zig build threads-test`, plus 1 `parallel_js`-only witness); `zig build threads-reference-audit` classifies the remaining reference-only files; negative promotion probes keep nearest blockers honest; no-GIL TSan corpus sweep; fuzzer gates. |
