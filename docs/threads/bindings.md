...
| Symbol | Location | What it is | Ruling | Notes / phase |
|---|---|---|---|---|
| `symbol_counter` | `src/interpreter.zig` | `std.atomic.Value(usize)` — monotonic id for unique `Symbol` property-key encodings (`"\x00s{d}"`). | **atomic, checked** | `makeSymbolObj` uses checked CAS minting, so cross-agent and shared-realm symbol creation cannot collide and counter exhaustion fails closed instead of wrapping into an earlier key. |
...
| `Generator.exec` / `started` / `done` / `suspended` / atomic `running` / `resume_mutex` | `src/vm.zig:166-198` | Persistent generator and async-function activation state. | **mutex + atomic resume ownership** | Ordinary generator entry uses `resume_mutex.tryLock` to preserve the re-entrant-resume `TypeError` and updates `running` under that lock. Async-function promise jobs acquire `running` with CAS and release-store it after the complete drive, so consecutive already-settled awaits drained by different shared-realm threads inherit prior activation writes without racing or holding a lock across arbitrary JS execution. |
...
| `group` / `group_used` | `src/agent.zig:66-70` | The `$262.agent` group: agent records, reports, broadcast storage/generation, and teardown state. | **locked** | All group fields are guarded by `group.mutex`; `group_used` is an atomic fast-path guard for reset/takeReport when no agent API was touched. Broadcast generations skip the idle `0` sentinel and agents ack the exact current generation, so the rendezvous invariant survives counter wrap. |
...
| `Context.js_threads` | `src/context.zig:1005`, `src/jsthread.zig` | Per-realm list of shared-realm `ThreadRecord`s, including the main record and spawned records. | **GIL/API-locked mutation; traced as realm roots** | Main-thread installation and spawned-thread construction reserve fixed-size capacity chunks before capacity-assumed appends. Spawned construction runs under `Gil.api_lock`, after live-cap/id checks and before id consumption; the API lock is trace-sensitive because the parallel collector takes it before tracing this list and the realm task queue. Teardown waits/joins records from the owner side. GC traces thread records through the context root path so pending joins, completion results, and thread-owned queues stay live. |
| `ThreadRecord.pending_joins` / `join_mutex` | `src/jsthread.zig:67` | Per-thread list of `Thread.asyncJoin()` promise observers waiting for completion. | **locked; traced through thread records** | `asyncJoin` appends under `join_mutex` and completion swaps the list out before settling promises outside the lock. Appends reserve fixed-size capacity chunks before capacity-assumed writes, and capacity growth is trace-sensitive so heap-cap recovery cannot collect back into the tracer's `join_mutex` acquisition. The list is rooted through `Context.js_threads` until completion publishes the snapshot, and completion temp-roots the snapshot promises while resolving/rejecting them. |
...
| `Gil.tasks` / `tasks_head` / `tasks_queued` | `src/gil.zig:28` | Per-realm run-loop task queue for `Lock.asyncHold` grant delivery. | **locked + atomic empty check** | Enqueue/dequeue mutate the queue under `Gil.api_lock`; writers publish `tasks_queued` from the locked queue length, while empty pump/quiescence checks read it lock-free. `tasks_head` gives FIFO dequeue without front-shifting the arena-backed task list, and task pumps copy larger bounded FIFO bursts under the lock before running grants outside it. Task-queue growth is trace-sensitive: heap-cap recovery fails closed instead of collecting while `traceGilTaskRoots` could reacquire `api_lock` to find queued `HoldJob` roots. GC traces queued `HoldJob` callback/promise roots through `jsthread.traceGilTaskRoots`; the currently dequeued burst is rooted separately through `Interpreter.current_hold_jobs` until every copied grant has run. |
...
| `LockRecord.pending` / `pending_head` | `src/jsthread.zig:547` | Per-lock FIFO of queued `Lock.asyncHold` / async condition reacquire grants. | **locked** | Protected by `LockRecord.mutex` with sync lock state. `pending_head` gives FIFO grant selection without front-shifting the arena-backed pending list; rare requeue-at-front uses the same helper and its own front stash. Both queues reserve fixed-size capacity chunks before capacity-assumed appends, so grant storms grow the lock-held arrays less often. Pending/front-queue growth is trace-sensitive: heap-cap recovery fails closed instead of collecting while the tracer may need the same lock to find queued `HoldJob` roots. GC traces pending `HoldJob` callback/promise roots through the owning Lock object's native private data, and job creation fires insertion barriers for active marks. No-fn `HoldJob`s embed their release state; the JS release function points at that per-grant state after delivery. |
| `Worker.inbox` / `Worker.outbox` `Channel.queue` / `queue_head` / `queued_bytes` / limits | `src/worker.zig:43` | Per-worker structured-clone message queues and live resource accounting. | **locked** | Protected by `Channel.mutex`; `queue_head` drains FIFO and dead prefixes compact before growth. Push checks live message/byte caps, transfers frame ownership only to a successful queue entry, unlocks before releasing rejected frames, and reports closed/cap/OOM rejection. Pop decrements exact live bytes; `close` preserves drain-then-stop semantics; `deinit` releases only unpopped streams. |
| `IterHelper.running` / `done` / cursor state | `src/value.zig`, `src/interpreter.zig` | Lazy Iterator Helper record for `Iterator.prototype.map/filter/take/drop/flatMap`, `Iterator.concat`, and `Iterator.zip*`. | **locked / snapshot before callout** | `IterHelper.lock` serializes `.next()` / `.return()` entry, `running` re-entrancy state, and `done` publication. The lock is released before invoking user callbacks, source `next`, or iterator `return`; while the helper is executing, peers observe `running` and fail with the same TypeError as re-entrant use. Runtime access to the helper's internal source/key/padding/result arrays uses locked element helpers or snapshots before user iterator calls/closing. Construction-time array fills remain private before publication. |
| `AsyncIterator` helper `IterHelper` state | `src/value.zig`, `src/interpreter.zig` | Async Iterator Helper records for `AsyncIterator.from` and `AsyncIterator.prototype.map/filter/take/drop/flatMap`. | **locked for entry/exit; no lock across awaits** | Async helpers share `IterHelper.lock`: `.next()` / `.return()` check `running` and publish `done` under the lock, then release it before awaiting source `next`, user callbacks, or async iterator `return`. Concurrent same-helper entry gets a rejected promise with the same TypeError policy as sync helpers. |
| `DisposableStack` / `AsyncDisposableStack` resource state | `src/interpreter.zig` | Hidden disposed flag, `\x00ds_res` resource list, and `[value, onDispose, kind]` tuples. | **locked / snapshot before callback** | The stack object's `elements_lock` is the native state mutex for disposed-state publication, resource-list push, `move()` transfer, and dispose/disposeAsync snapshot+clear. The lock is released before user disposal callbacks and async awaits. Resource tuples/lists use internal locked element helpers, and same-stack concurrent push after disposal starts observes disposed and throws `ReferenceError`. |
| `Gil.prop_waiters` / `Gil.prop_async` / `Gil.prop_mutex` | `src/gil.zig:31-40` | Per-realm property-mode `Atomics.wait` and `waitAsync` waiter queues plus their table mutex. | **locked** | Protected by `Gil.prop_mutex`, not process-global; independent `enable_threads` contexts cannot race or cross-notify each other. Sync wait and waitAsync revalidate the property value under this mutex immediately before enqueueing, closing the store+notify lost-wakeup window. Property waiter metadata uses the `Gil.prop_alloc` context allocator in real threaded contexts, so `heap_limit_bytes` covers sync ticket keys, async tickets, table backing storage, and settlement scratch; standalone unit-test `Gil` values default to the page allocator. Both waiter tables reserve fixed-size capacity chunks before capacity-assumed appends, and growth while `prop_mutex` is held is trace-sensitive so recovery fails closed rather than collecting while `gc.zig` could reacquire the same mutex to trace `prop_async` roots. Notify and timeout polling compact async tickets in one pass under the mutex, realm teardown frees abandoned async tickets by linear scan, and promise resolution happens after releasing the mutex. |
| `CondRecord.queue` / `CondRecord.mutex` | `src/jsthread.zig:392-399` | Per-condition FIFO for sync and async `Condition` waiters. | **locked** | Protected by `CondRecord.mutex`; sync wait registers before releasing the associated `Lock`, and `notify` marks/broadcasts under the same mutex. The FIFO head cursor skips timed-out or terminated sync waiters that were marked canceled, so timeout/termination cleanup does not middle-remove queue entries. Appends reserve fixed-size capacity chunks before capacity-assumed writes, so waiter bursts grow the lock-held queue less often. Sync ticket allocation and condition-queue growth are trace-sensitive: heap-cap recovery fails closed rather than collecting while the condition tracer could reacquire the same queue mutex. Async waiters hand off to the `Lock.asyncHold` grant path outside JS execution. GC traces async waiter promises through the Condition object's native private data. |
...
| `Context.c_api_handles` | `src/context.zig:1000` | Per-context counted `JSValueProtect` roots for GC-enabled C-API handles. | **realm-locked under parallel GC** | Mutated only through context-affine C-API calls, but the mid-script parallel collector also traces the table. `JSValueProtect` / `JSValueUnprotect` guard mutations with `realm_lock` when `parallel_js` is enabled; that lock is trace-sensitive so handle-table growth cannot recover allocation by collecting back into it. Protect deduplicates counted handles, rejects protection-count overflow instead of wrapping, and reserves fixed-size capacity chunks before capacity-assumed appends. `JSValueRef`/`JSObjectRef` boxes keep their `Value` first and carry an owner-context tag: context-taking APIs accept sibling-realm handles only inside the same `JSContextGroupRef` and reject foreign groups. Shared arena groups need no per-realm protection entry because the value remains allocated for the VM lifetime. `JSValueProtect` only controls GC reachability, not cross-thread access. |
...
| `Context.private_name_serial` | `src/context.zig`, `src/interpreter.zig` | Per-context source for unique class private-name storage keys. | **atomic, checked** | Parallel class evaluation mints keys with checked CAS. Exhaustion fails closed instead of wrapping, so distinct classes cannot alias private brands through a raced or reused serial. Standalone interpreters use the same checked policy on their local serial. |
| `Context.active_interpreters` | `src/context.zig:1012` | Per-context root list for interpreters currently evaluating, draining, or publishing roots. | **locked** | `pushActiveInterpreter` / `popActiveInterpreter` and GC iteration serialize with `active_interp_lock`. The lock is trace-sensitive, so registry growth cannot recover allocation by collecting back into the tracer's acquisition. Push reserves fixed-size capacity chunks before capacity-assumed append; pop uses swap removal because root order is not observable. GC uses this list at quiescent and parallel publication checkpoints to trace active execution roots. |
...
| `Object.elements_lock` + element storage | `src/value.zig`, `src/interpreter.zig`, `src/structured_clone.zig`, `src/c_api.zig`, `src/vm.zig` | Per-object lock for `elements: ArrayListUnmanaged(Value)`, used by arrays, Map/Set data, iterator cursor cells, and engine tuples. | **locked; residual direct paths classified** | `Object.lockElements` / `appendElement` / `appendElementIfLen` / `appendInternalElement` / `appendArrayHole` / `elementAt` / `setElementAt` / dense-array helpers are the synchronization funnel. Central dense-array get/set/delete/length, array-literal ordinary/hole appends, JSON array serialization length snapshots, JSON parse and Object enumerable/name/symbol result-array construction, VM/interpreter fresh result/argument/template-array appends, RegExp match/indices result arrays, iterator toArray and entry pair arrays, iterator-helper private-record appends, Reflect.ownKeys arrays, destructuring rest arrays, groupBy bucket arrays, Intl supported-values/Locale/format result arrays, packed reverse/sort/splice fast paths, descriptor reflection/definition paths, own-key and `for-in` dense-index enumeration, array destructuring/index-iteration length checks, `hasProperty` indexed chain checks, array object-spread reads, Array-method result appends, Promise combinator result arrays, packed array/call spread, `CreateListFromArrayLike`, VM spread-call and synthesized default-super argument snapshots, `Object.groupBy` / `Map.groupBy` packed-array collection snapshots, Intl canonical locale-list reads/appends, DisposableStack resource tuple/list access, Map/Set helper methods, Map/Set `forEach` per-slot snapshots, Set operation snapshots/live-index reads, native Set helper scans, Map/Set cursors, C-API array construction/indexed reads, structuredClone transfer-list validation, and structured-clone array/Map/Set serialization/deserialization now use locked helpers or locked snapshots before running JS/recursing. WeakMap/WeakSet dead-key pruning and FinalizationRegistry ready-record marking in the GC after-weak pass take the same lock used by register/unregister/automatic cleanup delivery. Dense/indexed helper writes also publish `indexed_own_seen`, a conservative cross-thread marker used by prototype-chain indexed-store guards in no-GIL mode. The remaining direct `elements.items` / raw element-storage sites are classified as lock-helper internals in `value.zig`; GC trace/finalize paths with stop-the-world or explicit lock rules; structured-clone snapshots that hold element locks and copy before recursion; lock-scoped Map/Set storage scans/appends and cursor/live-index helpers; and iterator-helper private setup/cleanup snapshots before publication or during abrupt close. Do not add new shared-realm public fast paths that borrow `elements.items` directly; use a locked helper/snapshot or document a narrower single-owner proof. Never hold this lock across a JS callback. |
...
| `value.ArrayBufferData` | `src/value.zig`, `src/interpreter.zig` | ArrayBuffer metadata and backing bytes. SharedArrayBuffer wrappers point at `SharedBufferStorage`; non-shared resizable buffers own `local_data`; C-API no-copy buffers borrow embedder bytes through an `ExternalBufferOwner`. | **locked** | Shared buffers use `SharedBufferStorage` with atomic refcount and atomic grow-only length. Non-shared `ArrayBufferData` has `lock`; `ArrayBuffer.prototype.resize` holds it while copying/publishing replacement `local_data`, typed-array and DataView read/write helpers hold it while borrowing the live slice, and bulk-copy operations such as `ArrayBuffer.prototype.slice` / `sliceToImmutable` re-resolve and copy through one locked source snapshot. This closes the no-GIL resize-vs-view/copy UAF window. No-copy buffers are fixed-length; their embedder remains responsible for synchronizing native byte access with JS execution. |
| `Context.external_buffer_owners` | `src/context.zig`, `src/c_api.zig`, `src/gc.zig` | Context-owned lifetime records for no-copy C-API ArrayBuffer backing stores. | **context-affine + atomic release** | C-API construction appends on the owner thread before publication. Each record has an atomic one-shot release gate: construction failure releases immediately, a GC object finalizer may release early, and context teardown releases any arena-mode remainder before destroying the records. The callback therefore runs exactly once without leaving a GC cell pointing at freed owner metadata. |
...
| `ThreadLocal` value map | `src/jsthread.zig` | Per-`ThreadLocal` map from JS thread id to stored `Value`. | **locked** | `ThreadLocal.value` get/set uses `TLRecord.map_lock`; GC traces stored object values through the ThreadLocal object's native private data so per-thread values are not hidden from collection. The map lock is trace-sensitive, so heap-cap allocation recovery fails closed while a mutator is growing or removing map entries instead of collecting and reacquiring the same lock from the tracer. Spawned threads remember the `ThreadLocal` records they touch and remove their OS-thread entry on normal exit or forced teardown. |
...
| Symbol | Location | What it is | Ruling | Notes |
|---|---|---|---|---|
| `c_allocator` | `zig-regex/src/c_api.zig:39` | `const c_allocator = std.heap.c_allocator` — the C-API allocator. | **locked** | `std.heap.c_allocator` (malloc/free) is internally thread-safe. The earlier audit note about this being a mutable `var` and referencing a nonexistent `zig_regex_get_last_error()` doc hook was fixed upstream in `zig-regex@5937fa7`; zig-js still does not link zig-regex's C API (it uses the Zig API), so this remains unreachable from engine threads. |
...
- Every current file-scope `var`, `pub var`, and `threadlocal` hit in `src/*.zig`
  has a ruling above.
- The original identity hazards are fixed: `math_prng` is threadlocal and
  entropy-seeded, while Symbol and class-private identity serials are atomic
  and checked against wraparound.
