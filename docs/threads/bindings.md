...
| `waiters`, `waiters_mutex`, `waiters_cond`, `waiters_used` | `src/agent.zig:288-293` | Typed-array `Atomics.wait` / `notify` / `waitAsync` waiter table. | **locked** | The table is guarded by `waiters_mutex`; `waiters_cond` wakes async harvesters; `waiters_used` is an atomic fast-path guard. Notify unlinks sync stack tickets before signaling, and async harvest/abandon stable-compact matching tickets in one pass while preserving FIFO order for remaining waiters. |
