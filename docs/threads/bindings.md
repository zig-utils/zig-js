...
| `DisposableStack` resource element storage | `src/interpreter.zig` | Internal `\x00ds_res` resource list and `[value, onDispose, kind]` tuples for `DisposableStack` / `AsyncDisposableStack`. | **element snapshots; record state tracked in #21** | Resource-list appends and tuple fills use internal locked element helpers. Disposal snapshots the resource list before invoking user disposal callbacks or awaiting async disposals, and tuple fields are read through locked element helpers. Same-stack disposed-state / push / move / dispose synchronization is broader than element storage and is tracked separately in #21. |
