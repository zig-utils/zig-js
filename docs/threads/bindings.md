...
| `Context.microtasks` | `src/context.zig:33` | The microtask queue (Promise reactions), shared with each `Interpreter` via pointer (`interpreter.zig:339`). | **per-thread** | Pinned PR-249 decision: each thread drains its own microtask queue; reactions run on the settling thread. One queue per agent (Layer A) and per JS thread (Layer B). The one cross-thread case — a finishing thread settling an `asyncJoin` enqueues a reaction into the *joiner's* queue — is GIL-serialized in the default engine and, under `parallel_js`, serialized by `Context.microtask_lock` (taken only across the queue append/pop, never across a job). |
