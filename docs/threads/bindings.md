...
| `IterHelper.running` / `done` / cursor state | `src/value.zig`, `src/interpreter.zig` | Lazy Iterator Helper record for `Iterator.prototype.map/filter/take/drop/flatMap`, `Iterator.concat`, and `Iterator.zip*`. | **locked for entry/exit; residual internal-array cleanup tracked in #18** | `IterHelper.lock` serializes `.next()` / `.return()` entry, `running` re-entrancy state, and `done` publication. The lock is released before invoking user callbacks, source `next`, or iterator `return`; while the helper is executing, peers observe `running` and fail with the same TypeError as re-entrant use. Internal helper arrays still need snapshot/helper cleanup before #18 closes. |
