...
| `Context.main_can_block` / `Context.max_js_threads` | `src/context.zig` | Stable host policies replacing the old process-global conformance knobs. | **per-context** | `main_can_block` models the VM's `[[CanBlock]]` bit; `max_js_threads` caps live shared-realm `Thread` records. Runners pass them through `Context.Options`, so concurrent contexts no longer race process-global knobs. |
