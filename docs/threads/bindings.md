...
| `Object.property_lock` + named property metadata | `src/value.zig`, `src/vm.zig` | Per-object lock for ordinary named-property shape publication, slot vector updates, accessor maps, attribute maps, and creation-order metadata. | **locked** | `Object.getOwn` / `setOwn` / `getAccessor` / `setAccessor` / `getAttr` / `setAttr` / `ownKeys` serialize through `property_lock`, and the VM's plain-property inline caches lock before reading/writing `shape` and `slots`. Dense `elements`, Map/Set element stores, and direct delete/rebuild side paths still rely on the GIL until Layer C funnels them through locked helpers or replaces the storage model. |
