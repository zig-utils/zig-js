...
- PR-249 coverage contains 230 promoted files out of 259 executable PR-249
  files: 228 in normal mode plus 2 `parallel_js`-only witnesses, covered by the
  sharded no-GIL
...
- `zig build midgc-profile` now makes convergence measurable without exposing
  ...
  peer root publications, and collector-side total/maximum pause time. Focused
  tests assert the attempt/outcome and abort-reason accounting identities, the
  max-versus-total bounds, and prove both running-peer publication and direct
  parked-peer observation paths execute.
...
- C-API protected-handle entries reserve fixed-size capacity chunks before
  ...
  are unchanged.
- Shared-realm `Thread` records reserve fixed-size capacity chunks before
  capacity-assumed appends for the main record and spawned records, so
