...
> **Sources.** Numbers come from `docs/.data/test262.json`; runner scope (subtrees, skips, exclusions, timeouts) from `conformance/test262.zig`. Regenerate the data with `bun scripts/gen-test262-data.ts` (the remaining Bun-based generator is tracked by #497).
...
The table below is populated only when `docs/.data/test262.json` was regenerated from a saved run that includes subtree lines. If the suite list is empty, do not invent rows; rerun or parse a saved transcript with `bun scripts/gen-test262-data.ts --from run.txt`.
...
> These figures are read from `docs/.data/test262.json`, regenerated by `bun scripts/gen-test262-data.ts`. Run the suite, regenerate, then `zig build docs-manifest-update` — the whole site reflects the new run.
...
- When using a saved transcript, keep `docs/.data/test262-run-*.txt` and `docs/.data/test262.json` paired. Otherwise regenerate `docs/.data/test262.json` directly with `bun scripts/gen-test262-data.ts`.
