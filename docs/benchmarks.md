...
- `zig build representative-benchmark` runs the versioned, dependency-free application-surface matrix from `docs/.data/representative-benchmark-matrix-v26.json`. V26 hash-inherits every V25 workload, representation, width, job count, checksum, execution mode, scored operation, attribution counter, timing boundary, measured-boundary occupancy rule, and logical/binary revision contract. It adds strict untimed native allocation replays and serial multi-row collection under one Home compilation; benchmark children never overlap, complete-process occupancy remains diagnostic, and the scored boundary retains the unchanged 60% gate. Accepted historical reports are not rewritten, capability boundaries remain explicit, lifecycle/no-JIT/string-indexing/external-suite evidence never enters repository-owned aggregates, and quick mode is validation only.
