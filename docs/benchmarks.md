...
- `zig build representative-benchmark` runs the versioned, dependency-free application-surface matrix from `docs/.data/representative-benchmark-matrix-v25.json`. V25 hash-inherits every V24 workload, representation, width, job count, checksum, execution mode, scored operation, attribution counter, allocation replay, timing boundary, and measured-boundary occupancy rule. It adds exact logical-versus-binary revision provenance for the identical one-commit measurement overlay; complete-process occupancy remains diagnostic and the scored boundary retains the unchanged 60% gate. Accepted historical reports are not rewritten, capability boundaries remain explicit, lifecycle/no-JIT/string-indexing/external-suite evidence never enters repository-owned aggregates, and quick mode is validation only.
