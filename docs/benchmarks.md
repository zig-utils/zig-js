...
- `zig build representative-benchmark` runs the versioned, dependency-free application-surface matrix from `docs/.data/representative-benchmark-matrix-v41.json`. V41 hash-inherits all scored V40 rows and adds one schema-v1 memory-pressure result after each invocation attribution and process snapshot, versioning the sidecar artifact as schema 15. Configuration and warmup carry `null`; the checkpoint result never enters timing scores, phase deltas, allocation replay, or publication decisions. V40 added the schema-v3 owned-memory inventory to all three attribution phases as diagnostic data. V39 re-pinned only the frontend parser runner after #948 froze nested-template-substitution and matched-parenthesis growth diagnostics outside representative coverage; V38 did the same for the zig-js comparison runner after private-environment diagnostics. V34 hash-inherits every V33 workload, representation, width, quick/full/publication job count, checksum, execution mode, scored operation, timing boundary, occupancy rule, native runner, mixed-tier replay signature, and attribution counter. Historical artifacts remain unchanged. Benchmark children never overlap, complete-process occupancy remains diagnostic, the scored boundary retains the 60% gate, historical reports are not rewritten, capability boundaries remain explicit, lifecycle/no-JIT/string-indexing/external-suite evidence never enters repository-owned aggregates, and quick mode is validation only.
