...
- `zig build representative-benchmark` runs the versioned, dependency-free application-surface matrix from `docs/.data/representative-benchmark-matrix-v24.json`. V24 hash-inherits every V23 workload, representation, width, job count, checksum, execution mode, scored operation, attribution counter, allocation replay, timing boundary, and publication ruling. It re-pins only the evidence collectors and native runners after adding measured-boundary CPU-time telemetry; complete-process occupancy is diagnostic, while the unchanged 60% quality threshold applies to the exact scored boundary. Accepted historical reports are not rewritten, capability boundaries remain explicit, lifecycle/no-JIT/string-indexing/external-suite evidence never enters repository-owned aggregates, and quick mode is validation only.
