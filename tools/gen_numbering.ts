...
// output bytes match theirs exactly. Run with Home's repository-tool runtime.
import { readText, utf8Bytes } from "./lib/home";
const src = readText(process.argv[2] || "test262/harness/testIntl.js");
const marker = "var numberingSystemDigits = {";
const start = src.indexOf(marker);
const end = start < 0 ? -1 : src.indexOf("\n};", start + marker.length);
if (start < 0 || end < 0) throw new Error("numberingSystemDigits not found");
...
const obj = parseNumberingSystemDigits(body);
...
let out = `// GENERATED by tools/gen_numbering.ts from test262 harness/testIntl.js.
