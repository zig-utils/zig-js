...
def render(rows: list[Row], lanes: list[int], raw_path: pathlib.Path | None, info: dict[str, str]) -> str:
    ...
    lines.extend([
        ...
        "- Shared mode prepares one zig-js shared realm and runs one unrecorded full-work shared `Thread` invocation outside the timer. Every timed sample then creates and joins the requested JavaScript `Thread`s. Its one-lane row is the scaling baseline.",
