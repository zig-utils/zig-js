...
export function runOne(binary: string, mode: string, workload: string, jobs: number, lanes: number): RunnerRow {
  ...
  const processWallNs = Math.round(timeMetric(completed.stderr, "real") * 1e9), userNs = Math.round(timeMetric(completed.stderr, "user") * 1e9), systemNs = Math.round(timeMetric(completed.stderr, "sys") * 1e9), occupancy = processCpuOccupancy(processWallNs, userNs, systemNs);
  requireValue(occupancy >= MINIMUM_PROCESS_CPU_OCCUPANCY, `benchmark process CPU occupancy ${(occupancy * 100).toFixed(1)}% is below ${(MINIMUM_PROCESS_CPU_OCCUPANCY * 100).toFixed(0)}%; transient competing work overlapped the invocation`);
  return { engine, mode, workload, lanes, jobs, elapsed_ns, checksum, process_wall_time_ns: processWallNs, process_cpu_user_ns: userNs, process_cpu_system_ns: systemNs, process_cpu_occupancy: occupancy, peak_rss_bytes: parsePeakRss(completed.stderr), retained_rss_bytes: mode === "single_observed" ? parseRetainedRss(completed.stdout, mode, workload, jobs) : null, allocations: allocationCounters?.[0] ?? null, allocated_bytes: allocationCounters?.[1] ?? null, instructions: counters.instructions.value, cycles: counters.cycles.value, energy_joules: counters.process_energy_nj.value / 1e9, thermal_state: `${thermal.before}->${thermal.after}` };
}
...
export function validateSampleQuality(samples: any[]): void {
  ...
  for (const sample of samples) {
    ...
    requireValue(Number.isFinite(quality.process_cpu_occupancy) && quality.process_cpu_occupancy >= MINIMUM_PROCESS_CPU_OCCUPANCY && quality.process_cpu_occupancy <= 1, "exact-parent sample process CPU occupancy is below the publication threshold");
    requireValue(quality.minimum_process_cpu_occupancy === MINIMUM_PROCESS_CPU_OCCUPANCY, "exact-parent sample process CPU occupancy policy drift");
  }
}
...
export function render(artifact: any): string { const metadata = artifact.metadata, summary = artifact.summary, efficiency = summary.efficiency, memoryRows = ["peak_rss_bytes", "retained_rss_bytes", "allocations", "allocated_bytes"].map((metric) => [metric, measuredMetricSummary(artifact.samples, metric)] as const).filter((entry) => entry[1] !== null).map(([metric, value]) => `| \`${metric}\` | ${value.parent_median} | ${value.candidate_median} | ${value.candidate_over_parent.toFixed(4)}x | ${(value.parent_rsd * 100).toFixed(2)}% | ${(value.candidate_rsd * 100).toFixed(2)}% |`); return [`# Exact-parent performance A/B — ${metadata.workload} (${metadata.mode}, ${metadata.lanes} lane(s))`, "", `- parent: \`${metadata.parent_revision}\``, `- candidate: \`${metadata.candidate_revision}\``, `- zig-gc: \`${metadata.zig_gc_revision}\``, `- zig-regex: \`${metadata.zig_regex_revision}\``, `- host class: \`${metadata.host_class}\``, `- material-change categories: ${efficiency.required_categories.map((value: string) => `\`${value}\``).join(", ")}`, `- sampling: ${metadata.samples} order-balanced pairs; no discarded samples`, `- process quality: every complete process used at least ${(metadata.minimum_process_cpu_occupancy * 100).toFixed(0)}% CPU occupancy; before/after snapshots reject persistent competing jobs`, `- timed boundary: ${metadata.timed_boundary}`, "", "| parent median | candidate median | candidate / parent | parent RSD | candidate RSD | assessment |", "| ---: | ---: | ---: | ---: | ---: | --- |", `| ${(summary.parent_wall_median_ns / 1e6).toFixed(3)} ms | ${(summary.candidate_wall_median_ns / 1e6).toFixed(3)} ms | ${summary.candidate_over_parent.toFixed(3)}x | ${(summary.parent_wall_rsd * 100).toFixed(2)}% | ${(summary.candidate_wall_rsd * 100).toFixed(2)}% | \`${summary.status}\` |`, "", "| memory/allocation metric | parent median | candidate median | candidate / parent | parent RSD | candidate RSD |", "| --- | ---: | ---: | ---: | ---: | ---: |", ...memoryRows, "", "| efficiency metric | parent median | candidate median | candidate / parent | parent RSD | candidate RSD |", "| --- | ---: | ---: | ---: | ---: | ---: |", ...["instructions", "cycles", "energy_joules"].map((metric) => { const value = efficiency.metrics[metric]; return `| \`${metric}\` | ${value.parent_median} | ${value.candidate_median} | ${value.candidate_over_parent.toFixed(4)}x | ${(value.parent_rsd * 100).toFixed(2)}% | ${(value.candidate_rsd * 100).toFixed(2)}% |`; }), ``, `Thermal states: ${efficiency.thermal_states.map((value: string) => `\`${value}\``).join(", ")}. Unmet category metrics: ${efficiency.unmet_metrics.length ? efficiency.unmet_metrics.map((value: string) => `\`${value}\``).join(", ") : "none"}. Efficiency evidence: \`${efficiency.stable ? "stable" : "blocked_or_diagnostic"}\`.`, "", "All input identities and checksums matched. Missing attribution values are encoded as unavailable with a reason, never as zero.", ""].join("\n"); }

function syntheticSamples(parent: number[], candidate: number[], schema: any): any[] { const result: any[] = []; parent.forEach((parentWall, pairSample) => { const walls: any = { parent: parentWall, candidate: candidate[pairSample] }, order = pairSample % 2 === 0 ? ["parent", "candidate"] : ["candidate", "parent"]; order.forEach((variant, position) => result.push(sampleRecord({ engine: "zig-js", mode: "single", workload: "representative_json", lanes: 1, jobs: 2200, elapsed_ns: walls[variant], checksum: 324952086, process_wall_time_ns: walls[variant], process_cpu_user_ns: Math.max(0, walls[variant] - 100), process_cpu_system_ns: 100, process_cpu_occupancy: 1, peak_rss_bytes: 10000000, retained_rss_bytes: null, allocations: null, allocated_bytes: null, instructions: 1000 + pairSample, cycles: 500 + pairSample, energy_joules: 0.2 + pairSample / 1000, thermal_state: "nominal->nominal" }, variant, pairSample, position, schema))); }); return result; }
...
export function selfTest(): void {
  ...
  validateSampleQuality(samples); const invalidQuality = structuredClone(samples); invalidQuality[0].quality.process_cpu_occupancy = 0.59; expectFailure(() => validateSampleQuality(invalidQuality), "below the publication threshold");
...
function main(): void {
  ...
  const samples = collect(positional[0], positional[1], options.mode, options.workload, options.jobs, options.lanes, options.expected_checksum, options.samples, schema); validateSampleQuality(samples); const metadata = { parent_revision: parentRevision, candidate_revision: candidateRevision, candidate_first_parent: resolveRevision(`${candidateRevision}^1`), zig_gc_revision: repositoryRevision(`${ROOT}/../zig-gc`), zig_regex_revision: repositoryRevision(`${ROOT}/../zig-regex`), zig_version: commandOutput(["zig", "version"]), os: commandOutput(["uname", "-a"]), hardware: `${commandOutput(["uname", "-m"])}; ${commandOutput(["sysctl", "-n", "machdep.cpu.brand_string"])}`, power: commandOutput(["pmset", "-g", "batt"], "unavailable").split(/\s+/).join(" "), host_class: options.host_class, material_change_categories: materialCategories, workload_source_sha256: sha256File(options.source), parent_binary_sha256: sha256File(positional[0]), candidate_binary_sha256: sha256File(positional[1]), samples: options.samples, minimum_process_cpu_occupancy: MINIMUM_PROCESS_CPU_OCCUPANCY, timed_boundary: options.timed_boundary, mode: options.mode, workload: options.workload, lanes: options.lanes, jobs: options.jobs, expected_checksum: options.expected_checksum };
