...
export function loadManifest(
  ...
  if (child.schema_version === 40) {
    requireValue(
      child.tier_attribution && typeof child.tier_attribution === "object" &&
        ...
        child.tier_attribution.artifact_schema_version === 14,
      "v40 must replace only the attribution artifact schema version",
    );
    requireValue(child.implemented_families_append === undefined && child.deferred_families_remove === undefined, "v40 re-pins the zig-js comparison runner only");
    requireValue(child.pending_metric_panels === undefined && child.completed_metric_panels === undefined, "v40 must inherit completed panel inventory unchanged");
    requireValue(child.instrumentation_overhead_integration === undefined, "v40 must inherit instrumentation_overhead_integration unchanged");
    const diagnostics = child.memory_inventory_diagnostics;
    requireValue(
      diagnostics && typeof diagnostics === "object" && diagnostics.owner_issue === 974 && diagnostics.snapshot_schema_version === 3 &&
        same(diagnostics.phases || [], ["configuration", "warmup", "invocation"]) &&
        diagnostics.scored === false && diagnostics.row_field === "memory",
    ...
    requireValue(
      ...
      "v40 replaces only the exact-parent native runners and publication boundary",
    );
    ...
    requireValue(
      ...
      "v40 native runner inventory drift",
    );
    for (let index = 0; index < replacement.native_runners.length; index += 1) {
      ...
      if (runner.path !== "bench/comparison_zig_js.zig")
        requireValue(runner.sha256 === inheritedRunners[index].sha256, `v40 must inherit ${runner.path} unchanged`);
    }
  ...
  if (child.schema_version === 39) {
    requireValue(child.tier_attribution === undefined, "v39 must inherit the scored attribution contract unchanged");
    requireValue(child.implemented_families_append === undefined && child.deferred_families_remove === undefined, "v39 re-pins the frontend native runner only");
    requireValue(child.pending_metric_panels === undefined && child.completed_metric_panels === undefined, "v39 must inherit completed panel inventory unchanged");
    for (const name of ["instrumentation_overhead_integration", "context_lifecycle_integration", "no_jit_integration", "string_indexing_integration"])
      requireValue(child[name] === undefined, `v39 must inherit ${name} unchanged`);
    const diagnostics = child.frontend_growth_diagnostics;
    requireValue(
      diagnostics && typeof diagnostics === "object" && diagnostics.owner_issue === 948 && diagnostics.payload_elements === 200000 &&
        same(diagnostics.families || [], ["representative_frontend_template_substitution_nested_", "representative_frontend_template_substitution_parentheses_"]) &&
        same(diagnostics.depths || [], [32, 64, 128, 256]),
      "v39 template-substitution growth inventory drift",
    );
    const replacement = child.exact_parent_integration;
    requireValue(
      replacement && typeof replacement === "object" && same(Object.keys(replacement).sort(), ["native_runners", "publication_boundary"]),
      "v39 replaces only the exact-parent native runners and publication boundary",
    );
    const inheritedRunners = inherited.exact_parent_integration.native_runners;
    requireValue(
      Array.isArray(replacement.native_runners) && Array.isArray(inheritedRunners) &&
        same(replacement.native_runners.map((runner: any) => runner.path), inheritedRunners.map((runner: any) => runner.path)),
      "v39 native runner inventory drift",
    );
    for (let index = 0; index < replacement.native_runners.length; index += 1) {
      const runner = replacement.native_runners[index];
      if (runner.path !== "bench/frontend_parse.zig")
        requireValue(runner.sha256 === inheritedRunners[index].sha256, `v39 must inherit ${runner.path} unchanged`);
    }
    const exactParent = { ...inherited.exact_parent_integration, ...replacement };
    const merged = {
      ...inherited,
  ...
  if (child.schema_version === 37) {
    ...
    requireValue(child.implemented_families_append === undefined && child.deferred_families_remove === undefined, "v37 re-pins the frontend native runner only");
    requireValue(child.pending_metric_panels === undefined && child.completed_metric_panels === undefined, "v37 must inherit completed panel inventory unchanged");
    for (const name of ["instrumentation_overhead_integration", "context_lifecycle_integration", "no_jit_integration", "string_indexing_integration"])
      requireValue(child[name] === undefined, `v37 must inherit ${name} unchanged`);
    const replacement = child.exact_parent_integration;
    requireValue(
      replacement && typeof replacement === "object" && same(Object.keys(replacement).sort(), ["native_runners", "publication_boundary"]),
      "v37 replaces only the exact-parent native runners and publication boundary",
    );
    const inheritedRunners = inherited.exact_parent_integration.native_runners;
    requireValue(
      Array.isArray(replacement.native_runners) && Array.isArray(inheritedRunners) &&
        same(replacement.native_runners.map((runner: any) => runner.path), inheritedRunners.map((runner: any) => runner.path)),
      "v37 native runner inventory drift",
    );
    for (let index = 0; index < replacement.native_runners.length; index += 1) {
      const runner = replacement.native_runners[index];
      if (runner.path !== "bench/frontend_parse.zig")
        requireValue(runner.sha256 === inheritedRunners[index].sha256, `v37 must inherit ${runner.path} unchanged`);
    }
    const exactParent = { ...inherited.exact_parent_integration, ...replacement };
    const merged = {
      ...inherited,
  ...
  if (child.schema_version === 36) {
    requireValue(child.tier_attribution === undefined, "v36 must inherit the scored attribution contract unchanged");
    requireValue(child.implemented_families_append === undefined && child.deferred_families_remove === undefined, "v36 re-pins the frontend native runner only");
    requireValue(child.pending_metric_panels === undefined && child.completed_metric_panels === undefined, "v36 must inherit completed panel inventory unchanged");
    for (const name of ["instrumentation_overhead_integration", "context_lifecycle_integration", "no_jit_integration", "string_indexing_integration"])
      requireValue(child[name] === undefined, `v36 must inherit ${name} unchanged`);
    const replacement = child.exact_parent_integration;
    requireValue(
      replacement && typeof replacement === "object" && same(Object.keys(replacement).sort(), ["native_runners", "publication_boundary"]),
      "v36 replaces only the exact-parent native runners and publication boundary",
    );
    const inheritedRunners = inherited.exact_parent_integration.native_runners;
    requireValue(
      Array.isArray(replacement.native_runners) && Array.isArray(inheritedRunners) &&
        same(replacement.native_runners.map((runner: any) => runner.path), inheritedRunners.map((runner: any) => runner.path)),
      "v36 native runner inventory drift",
    );
    for (let index = 0; index < replacement.native_runners.length; index += 1) {
      const runner = replacement.native_runners[index];
      if (runner.path !== "bench/frontend_parse.zig")
        requireValue(runner.sha256 === inheritedRunners[index].sha256, `v36 must inherit ${runner.path} unchanged`);
    }
    const exactParent = { ...inherited.exact_parent_integration, ...replacement };
    const merged = {
      ...inherited,
  ...
  if (child.schema_version === 35) {
    requireValue(child.tier_attribution === undefined, "v35 must inherit the scored attribution contract unchanged");
    requireValue(child.implemented_families_append === undefined && child.deferred_families_remove === undefined, "v35 re-pins the frontend native runner only");
    requireValue(child.pending_metric_panels === undefined && child.completed_metric_panels === undefined, "v35 must inherit completed panel inventory unchanged");
    for (const name of ["instrumentation_overhead_integration", "context_lifecycle_integration", "no_jit_integration", "string_indexing_integration"])
      requireValue(child[name] === undefined, `v35 must inherit ${name} unchanged`);
    const replacement = child.exact_parent_integration;
    requireValue(
      replacement && typeof replacement === "object" && same(Object.keys(replacement).sort(), ["native_runners", "publication_boundary"]),
      "v35 replaces only the exact-parent native runners and publication boundary",
    );
    const inheritedRunners = inherited.exact_parent_integration.native_runners;
    requireValue(
      Array.isArray(replacement.native_runners) && Array.isArray(inheritedRunners) &&
        same(replacement.native_runners.map((runner: any) => runner.path), inheritedRunners.map((runner: any) => runner.path)),
      "v35 native runner inventory drift",
    );
    for (let index = 0; index < replacement.native_runners.length; index += 1) {
      const runner = replacement.native_runners[index];
      if (runner.path !== "bench/frontend_parse.zig")
        requireValue(runner.sha256 === inheritedRunners[index].sha256, `v35 must inherit ${runner.path} unchanged`);
    }
    const exactParent = { ...inherited.exact_parent_integration, ...replacement };
    const merged = {
      ...inherited,
