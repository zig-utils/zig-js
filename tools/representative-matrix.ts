...
export const DEFAULT_MANIFEST = ROOT + "/docs/.data/representative-benchmark-matrix-v26.json";
...
export function loadManifest(
  ...
  requireValue(child.schema_version >= 2 && child.schema_version <= 26, "unsupported representative matrix schema");
  ...
  const inherited = loadManifest(
    ...
    deferIntegrationValidation || child.schema_version === 24 || child.schema_version === 25 || child.schema_version === 26,
  );
  ...
  if (child.schema_version === 24 || child.schema_version === 25 || child.schema_version === 26) {
...
export function validate(manifest: any, root = ROOT): void {
  requireValue(manifest.schema_version >= 1 && manifest.schema_version <= 26, "unsupported representative matrix schema");
  ...
  if (manifest.schema_version >= 16) {
    ...
    if (manifest.schema_version >= 24) {
      ...
      if (manifest.schema_version >= 25) requireValue(efficiency.scored_integration.binary_provenance === "identical_one-commit_measurement_overlay", "V25 binary provenance policy drift");
