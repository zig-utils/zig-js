...
def readme_scorecard(rows, metadata: dict[str, str], report_link: str, raw_link: str) -> str:
    ...
    lines = [
        ...
        "",
        f"Latest accepted [report]({report_link}) · [{len(rows):,} raw samples]({raw_link}). Equal-work checks, alternating order, dispersion limits, and a 50 ms timing floor are enforced by the harness.",
        "",
