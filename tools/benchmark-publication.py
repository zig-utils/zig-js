...
def readme_scorecard(rows, metadata: dict[str, str], report_link: str, raw_link: str) -> str:
    ...
    lines = [
        ...
        "",
        f"The latest [full report]({report_link}) preserves all [{len(rows):,} raw timing samples]({raw_link}) from clean zig-js commit `{zig_revision}`, zig-gc `{gc_revision}`, and zig-regex `{regex_revision}` on {host}. The run used Zig `{metadata['Zig']}`, {metadata['JavaScriptCore']}, and {power_source} ({power_state}). Every full-run median exceeds the 50 ms timing floor; equal work/checksums, alternating runner order, and per-row dispersion are validated by the harness.",
        "",
    ...
    for workload, scaling in zip(workloads, shared_scaling):
        count = jobs[workload]
        zig = median_ns(grouped, ("zig-js", "single", workload, 1, count))
        jsc = median_ns(grouped, ("JavaScriptCore", "single", workload, 1, count))
