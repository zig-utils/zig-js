...
- most native optimizer object writes, eval/spread/tail-call forms, dynamic-finally dispatch, and additional architecture backends. AArch64 executes named/computed reads, method and ordinary calls, construction, and `to_numeric` through precisely rooted callbacks, including exact catch/finally continuations without replay; see [optimizer status](docs/optimizing-jit.md).
