...
- VM/tree-walker numbers below come from [docs/.data/bench-2026-07-04.txt](docs/.data/bench-2026-07-04.txt); the JSC comparison comes from the [July 17, 2026 VM-run-lease report](docs/.data/benchmark-comparison-2026-07-17-vm-run-leases.md) and its [1,540 raw timing samples](docs/.data/benchmark-comparison-2026-07-17-vm-run-leases.tsv).
...
| --- | ---: | ---: | ---: | ---: |
| `arithmetic` | 82.620 | 351.164 | 4.25x | 6.07x |
| `properties` | 95.347 | 291.019 | 3.05x | 5.36x |
| `polymorphic_properties` | 97.700 | 209.031 | 2.14x | 5.58x |
| `object_churn` | 89.223 | 120.483 | 1.35x | 0.81x |
| `arrays` | 91.493 | 152.330 | 1.66x | 2.55x |
| `direct_calls` | 77.444 | 119.702 | 1.55x | 6.43x |
| `method_calls` | 89.819 | 137.087 | 1.53x | 4.40x |
| `closure_calls` | 90.125 | 209.932 | 2.33x | 7.23x |
| `arguments_calls` | 91.467 | 309.319 | 3.38x | 5.67x |
| `fibonacci` | 92.864 | 465.310 | 5.01x | 4.27x |

zig-js wins 10/10 direct rows, 9/10 maximum-lane warmed-independent rows, and 10/10 maximum-lane cold-lifecycle rows. The geometric-mean throughput lead is 2.38x direct, 2.36x warmed-independent, and 2.61x cold-lifecycle; shared-realm scaling is 4.24x at 8 lanes.
<!-- benchmark-comparison:end -->

Object instances occupy a 128-byte GC slab (`96` bytes of payload and `128` raw bytes including collector metadata). One lazy storage wrapper owns cold/exotic state, external named-slot metadata, dense/internal element metadata, and backing-allocator bookkeeping; a plain object with four or fewer named properties keeps its values entirely inline and allocates none of those side states. In the current matrix, object churn is 89.223 versus 120.483 ms direct and 163.456 versus 191.581 ms across eight cold lifecycles, both favoring zig-js. The warmed eight-context row is the matrix's one JSC win at 210.643 versus 180.245 ms. Shared object churn reaches 1,648.219 ms at eight lanes, 0.81x scaling, and 13.22% RSD, so it remains a clear contention target.
...
files**, classified as 431 private (**153 implemented / 278 pending**), 15
...
The first 153 private exports—encoded identity/cell equality, truthiness,
...
CommonAbortReason-to-DOMException conversion, plus seven
...
pages, and leaves later calls on safe bytecode/recompilation fallback. The
...
The 152-symbol combined fixture covers sibling realms,
...
separately compiled fixtures pass 20 real cell kinds for each. All 153
...
437 symbols from 54 `src/jsc` files: 421 private (**147 implemented / 274
