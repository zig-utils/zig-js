...
- automatic/concurrent compaction, a moving nursery, and optimizing-tier execution beyond handler-aware numeric SSA (throw/return completions resume through bytecode `finally`; calls still reject), resumable side exits, and poll-safe loop OSR with parallel backedges and one equal-cost inner branch; see the [optimizer status](docs/optimizing-jit.md).
