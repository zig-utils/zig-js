...
| --- | ---: | ---: | ---: |
| arithmetic | 76.500 | 350.458 | 4.58x |
| properties | 85.833 | 291.590 | 3.40x |
| polymorphic properties | 80.304 | 199.006 | 2.48x |
| object churn | 121.638 | 118.874 | 0.98x |
| arrays | 79.563 | 149.144 | 1.87x |
| direct calls | 58.278 | 120.848 | 2.07x |
| method calls | 60.132 | 136.643 | 2.27x |
| closure calls | 62.488 | 188.349 | 3.01x |
| arguments calls | 66.725 | 294.830 | 4.42x |
| Fibonacci | 84.692 | 451.621 | 5.33x |
...
zig-js wins 9 of 10 direct single-context rows. Across the complete 10-workload matrix, its geometric-mean throughput lead is 2.74x in direct single-context mode and 2.76x at eight warmed independent contexts. Mode-local eight-lane scaling is 5.47x for zig-js and 5.35x for JSC; the symmetric cold lifecycle has a 2.65x zig-js throughput lead and scales 5.38x and 5.26x respectively. Shared-realm scaling is 4.00x by geometric mean. Object churn remains the only direct loss, but the gap is now 2.3%: 121.638 ms for zig-js versus 118.874 ms for JSC. The weak-pass gate proves that a context has no weak semantic state before skipping ephemeron, weak-slot, and after-weak processing. An exact pre-hook/current-pair A/B measured 24.4% lower direct object-churn time and 38–40% lower shared time at every lane count, with matching checksums. Against the previous full publication, direct object churn fell 18.5% and shared 1/2/4/8-lane wall time fell 38.2%/38.4%/40.7%/24.3%. Shared object-churn scaling is still below 1x and its 14–45% RSD makes it the next explicit GC/concurrency target. See [Performance Benchmarks](docs/benchmarks.md) for all lane counts, cold results, exact timed boundaries, allocator choice, dispersion, caveats, reproduction, and raw evidence.
