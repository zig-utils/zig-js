...
- **GC performance** - `zig build gc-profile` compares arena, explicit-GC,
  ...
  collection/destroy lookup costs bounded. Single-mutator GC object side stores
  now allocate directly from the context allocator instead of round-tripping
  through that cell-slab classifier, while true-parallel contexts keep the
  synchronized backing wrapper. Context teardown now enters a slab
  bulk-teardown mode so per-cell frees do not rebuild freelists or reclassify
  bucket ownership immediately before whole chunks are released; keep using the
  profile to drive nursery/generational work and further lifecycle reductions
