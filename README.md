...
- **GC performance** - `zig build gc-profile` compares arena, explicit-GC,
  ...
  chunk lists to keep collection/destroy lookup costs bounded. Context teardown
  now enters a slab bulk-teardown mode so per-cell frees do not rebuild freelists
  immediately before whole chunks are released; keep using the profile to drive
